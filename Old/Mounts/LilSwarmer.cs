...
namespace Antiaris.Mounts
{
	public class LilSwarmer : ModMountData
	{
		...
		{
			mountData.buff = ModContent.BuffType<LilSwarmer>();
		...
		{
			if (Math.Abs(player.velocity.X) >= 4.0)
			{
				Rectangle rect = player.getRect();
				Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, 19, 0.0f, 0.0f, 0, new Color(), 1f);
				Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, 32, 0.0f, 0.0f, 0, new Color(), 1f);
			}
		}
	}
}