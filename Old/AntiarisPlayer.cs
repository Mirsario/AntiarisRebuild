...
using Terraria.UI;
using Antiaris.Projectiles.Miscellaneous;
using Antiaris.Items.Miscellaneous;
using Antiaris.Items.Weapons.Thrown;
using Antiaris.Items.Consumables.Crates;
...
namespace Antiaris
{
	public class AntiarisPlayer : ModPlayer
	{
		...
		{
			if (AntiarisMod.hideTracker.JustPressed)
		...
		public override void UpdateBiomeVisuals()
		{
			...
			for (int k = 0; k < 200; k++)
			{
				NPC Queen = Main.npc[k];
				if (Queen.type == ModContent.NPCType<AntlionQueen>() && Queen.life < Queen.lifeMax * 0.15f)
				{
					Antlion = true;
				}
				if (Queen.type == ModContent.NPCType<AntlionQueen>() && !Queen.active)
				{
					Antlion = false;
				}
			}
			...
			player.ManageSpecialBiomeVisuals("Antiaris:TimeSky", (ModContent.GetInstance<AntiarisWorld>().frozenTime && !inversion));
			player.ManageSpecialBiomeVisuals("Antiaris:AntlionQueen", Antlion);
			player.ManageSpecialBiomeVisuals("Antiaris:Corruption", !ModContent.GetInstance<AntiarisWorld>().DownedTowerKeeper && (player.ZoneCorrupt || player.ZoneCrimson) && (!NPC.AnyNPCs(ModContent.NPCType<TowerKeeper>()) || !NPC.AnyNPCs(ModContent.NPCType<TowerKeeper2>()) || !NPC.AnyNPCs(ModContent.NPCType<TowerKeeperNonActive>())));
			player.ManageSpecialBiomeVisuals("Antiaris:DeadlyJones", NPC.AnyNPCs(ModContent.NPCType<DeadlyJones>()));
		...
		public override void DrawEffects(PlayerDrawInfo drawInfo, ref float r, ref float g, ref float b, ref float a, ref bool fullBright)
		{
			...
			if (player.buffType.Contains(ModContent.BuffType<BadgersResolve>()))
			{
				r *= 1f;
				g *= 0.8f;
				b *= 0.1f;
		...
		public override bool PreKill(double damage, int hitDirection, bool pvp, ref bool playSound, ref bool genGore, ref PlayerDeathReason damageSource)
		{
			...
			if (electrified && damage == 10.0 && hitDirection == 0 && damageSource.SourceOtherIndex == 8)
				damageSource = PlayerDeathReason.ByOther(10);
			if (injured)
				damageSource = PlayerDeathReason.ByCustomReason(InjuredDeath);
			if (!player.buffType.Contains(ModContent.BuffType<LivingontheEdge>()) && davyJonessPendant)
			{
				player.statLife += 200;
				player.AddBuff(ModContent.BuffType<LivingontheEdge>(), 10800);
				return false;
			}
			return true;
		}
		...
		public override void PreUpdate()
		{
			...
			if (Main.raining)
			{
				...
				{
					TextureAssets.Rain = ModContent.GetTexture("Antiaris/Miscellaneous/Rain_Crimson");
				}
				else if (Main.LocalPlayer.ZoneCorrupt)
				{
					TextureAssets.Rain = ModContent.GetTexture("Antiaris/Miscellaneous/Rain_Corruption");
				}
				else if (Main.LocalPlayer.ZoneDesert || Main.LocalPlayer.ZoneUndergroundDesert)
				{
					TextureAssets.Rain = ModContent.GetTexture("Antiaris/Miscellaneous/Rain_Desert");
				}
				else if (Main.LocalPlayer.ZoneJungle)
				{
					TextureAssets.Rain = ModContent.GetTexture("Antiaris/Miscellaneous/Rain_Jungle");
				}
				else if (Main.LocalPlayer.ZoneHoly)
				{
					TextureAssets.Rain = ModContent.GetTexture("Antiaris/Miscellaneous/Rain_Hallow");
				}
				else
				{
					TextureAssets.Rain = Main.instance.OurLoad<Texture2D>(string.Concat(new object[] { "Images", Path.DirectorySeparatorChar, "Rain" }));
				}
			...
			if (player.breath < 40)
			{
				...
				for (int i = 0; i < 58; i++)
				{
					if (player.inventory[i].stack > 0 && player.inventory[i].type == ModContent.ItemType<FilledOxygenTank>())
					{
						Item item = player.inventory[i];
						int Removed = Math.Min(item.stack, 1);
						item.stack -= Removed;
						if (item.stack <= 0)
							item.SetDefaults();
			...
			int[] QItems =
			{
				ModContent.ItemType<StolenPresent>(),
				ModContent.ItemType<SpiderMass>(),
				ModContent.ItemType<SilkScarf>(),
				ModContent.ItemType<PlatinumApple>(),
				ModContent.ItemType<OldCompass>(),
				ModContent.ItemType<Necronomicon>(),
				ModContent.ItemType<MonsterSkull>(),
				ModContent.ItemType<MagicalAmulet>(),
				ModContent.ItemType<HarpyEgg>(),
				ModContent.ItemType<GoldenApple>(),
				ModContent.ItemType<GlacialCrystal>(),
				ModContent.ItemType<DemonWingPiece>(),
				ModContent.ItemType<Coconut>(),
				ModContent.ItemType<AmuletPiece3>(),
				ModContent.ItemType<AmuletPiece2>(),
				ModContent.ItemType<AmuletPiece1>(),
				ModContent.ItemType<AdventurersFishingRodPart3>(),
				ModContent.ItemType<AdventurersFishingRodPart2>(),
				ModContent.ItemType<AdventurersFishingRodPart1>(),
				ModContent.ItemType<AdventurersFishingRod>(),
				ModContent.ItemType<AdventurerChest>(),
				ModContent.ItemType<EmeraldShard>()
			};
			for (int i = 0; i < 59; i++)
			{
				if (player.statLife <= 0)
				{
					...
					{
						int item = Item.NewItem((int)player.position.X, (int)player.position.Y, player.width, player.height, player.inventory[i].type, 1, false, 0, false, false);
						Main.item[item].netDefaults(player.inventory[i].netID);
						Main.item[item].Prefix(player.inventory[i].prefix);
						Main.item[item].stack = player.inventory[i].stack;
						Main.item[item].velocity.Y = Main.rand.Next(-20, 1) * 0.2f;
						Main.item[item].velocity.X = Main.rand.Next(-20, 21) * 0.2f;
						Main.item[item].noGrabDelay = 100;
						Main.item[item].newAndShiny = false;
						player.inventory[i] = new Item();
						foreach (int item2 in QItems)
			...
			if (Main.myPlayer == player.whoAmI)
			{
				if (player.statLife > 0)
					checkGhost = true;
				if (player.statLife <= 0 && player.difficulty != 2)
				{
					if (checkGhost)
						NPC.NewNPC((int)player.Center.X, (int)player.Center.Y, ModContent.NPCType<Ghost>(), 0, Main.myPlayer, 0f, 0f, 0f, 255);
					checkGhost = false;
				}
			}
		...
		public override void PostUpdate()
		{
			...
			if (ModContent.GetInstance<AntiarisWorld>().frozenTime && Main.LocalPlayer.FindBuffIndex(ModContent.BuffType<FrozenTime>()) == -1)
			...
			if (!player.dead)
			{
				if (!swarmerS)
					return;
				if (player.ownedProjectileCounts[ModContent.ProjectileType<AntlionSummon>()] <= 2 && timer % 540 == 0)
				{
					AntiarisHelper.CreateDust(player, 64, 40);
					Projectile.NewProjectile(player.position, Vector2.Zero, ModContent.ProjectileType<AntlionSummon>(), 0, 0, player.whoAmI);
				}
			}
			if (player.mount.Active && player.mount.Type == ModContent.MountType<EmeraldSlime>() && player.wetSlime == 0 && player.velocity.Y > 0f)
			{
				...
				{
					bool flag = false;
					if (player.mount.Active && player.mount.Type == ModContent.MountType<EmeraldSlime>() && player.wetSlime > 0)
					{
						flag = true;
					}
					bool flag2;
					...
					else if ((player.sliding || player.velocity.Y == 0f || flag || player.jumpAgainCloud || player.jumpAgainSandstorm || player.jumpAgainBlizzard || player.jumpAgainFart || player.jumpAgainSail || player.jumpAgainUnicorn || (player.wet && player.accFlipper && (!player.mount.Active || !player.mount.Cart))) && (player.releaseJump || (player.autoJump && (player.velocity.Y == 0f || player.sliding))))
					{
						...
						if (player.velocity.Y == 0f || flag2 || player.sliding || flag)
						{
							...
							if (player.sliding)
							{
								player.velocity.X = 3 * -(float)player.slideDir;
							}
						}
						else if (flag3)
						{
						...
						else if (flag6)
						{
							...
							for (j = 0; j < 30; j++)
							{
								num5 = Dust.NewDust(new Vector2(player.position.X, player.position.Y + num4), player.width, 12, 253, player.velocity.X * 0.3f, player.velocity.Y * 0.3f, 100, default, 1.5f);
								if (j % 2 == 0)
								{
									expr_795_cp_0 = Main.dust[num5];
									expr_795_cp_0.velocity.X = expr_795_cp_0.velocity.X + Main.rand.Next(30, 71) * 0.1f;
								}
								else
								{
									expr_7C5_cp_0 = Main.dust[num5];
									expr_7C5_cp_0.velocity.X = expr_7C5_cp_0.velocity.X - Main.rand.Next(30, 71) * 0.1f;
								}
								expr_7F3_cp_0 = Main.dust[num5];
								expr_7F3_cp_0.velocity.Y = expr_7F3_cp_0.velocity.Y + Main.rand.Next(-10, 31) * 0.1f;
								Main.dust[num5].noGravity = true;
								Main.dust[num5].scale += Main.rand.Next(-10, 41) * 0.01f;
								Main.dust[num5].velocity *= Main.dust[num5].scale * 0.7f;
								value = new Vector2(Main.rand.Next(-100, 101), Main.rand.Next(-100, 101));
								value.Normalize();
								value *= Main.rand.Next(81) * 0.1f;
							}
						}
						else if (flag5)
						{
							...
							for (k = 0; k < 10; k++)
							{
								num7 = Dust.NewDust(new Vector2(player.position.X - 34f, player.position.Y + num6 - 16f), 102, 32, 188, -player.velocity.X * 0.5f, player.velocity.Y * 0.5f, 100, default, 1.5f);
							...
							num8 = Gore.NewGore(new Vector2(player.position.X + player.width / 2 - 16f, player.position.Y + num6 - 16f), new Vector2(-player.velocity.X, -player.velocity.Y), Main.rand.Next(435, 438), 1f);
							...
							num8 = Gore.NewGore(new Vector2(player.position.X - 36f, player.position.Y + num6 - 16f), new Vector2(-player.velocity.X, -player.velocity.Y), Main.rand.Next(435, 438), 1f);
							Main.gore[num8].velocity.X = Main.gore[num8].velocity.X * 0.1f - player.velocity.X * 0.1f;
							Main.gore[num8].velocity.Y = Main.gore[num8].velocity.Y * 0.1f - player.velocity.Y * 0.05f;
							num8 = Gore.NewGore(new Vector2(player.position.X + player.width + 4f, player.position.Y + num6 - 16f), new Vector2(-player.velocity.X, -player.velocity.Y), Main.rand.Next(435, 438), 1f);
							Main.gore[num8].velocity.X = Main.gore[num8].velocity.X * 0.1f - player.velocity.X * 0.1f;
							Main.gore[num8].velocity.Y = Main.gore[num8].velocity.Y * 0.1f - player.velocity.Y * 0.05f;
						}
						else if (flag7)
						{
							...
							for (l = 0; l < 5; l++)
							{
								for (num10 = 0f; num10 < 14f; num10 += 1f)
								{
									...
									vector = Vector2.UnitY.RotatedBy(num10 * 6.28318548f / 14f + num9, default);
									vector *= 0.2f * l;
						...
						else
						{
							...
							for (m = 0; m < 10; m++)
							{
								num12 = Dust.NewDust(new Vector2(player.position.X - 34f, player.position.Y + num11 - 16f), 102, 32, 16, -player.velocity.X * 0.5f, player.velocity.Y * 0.5f, 100, default, 1.5f);
							...
							num13 = Gore.NewGore(new Vector2(player.position.X + player.width / 2 - 16f, player.position.Y + num11 - 16f), new Vector2(-player.velocity.X, -player.velocity.Y), Main.rand.Next(11, 14), 1f);
							...
							num13 = Gore.NewGore(new Vector2(player.position.X - 36f, player.position.Y + num11 - 16f), new Vector2(-player.velocity.X, -player.velocity.Y), Main.rand.Next(11, 14), 1f);
							Main.gore[num13].velocity.X = Main.gore[num13].velocity.X * 0.1f - player.velocity.X * 0.1f;
							Main.gore[num13].velocity.Y = Main.gore[num13].velocity.Y * 0.1f - player.velocity.Y * 0.05f;
							num13 = Gore.NewGore(new Vector2(player.position.X + player.width + 4f, player.position.Y + num11 - 16f), new Vector2(-player.velocity.X, -player.velocity.Y), Main.rand.Next(11, 14), 1f);
							Main.gore[num13].velocity.X = Main.gore[num13].velocity.X * 0.1f - player.velocity.X * 0.1f;
							Main.gore[num13].velocity.Y = Main.gore[num13].velocity.Y * 0.1f - player.velocity.Y * 0.05f;
						}
					}
			...
			for (int i = 0; i < items.Count; i++)
			{
				...
				switch (item.type)
				{
					...
					case ItemID.CopperPickaxe:
						item.type = ModContent.ItemType<WoodenPickaxe>();
						break;
		...
		public override void PostUpdateEquips()
		{
			...
			for (int j = 0; j < Main.npc.Length; j++)
			{
				...
				for (int h = 0; h < target.buffType.Length; h++)
				{
					...
					if (type == ModContent.BuffType<Prey>())
					{
			...
			if (mechanicalHeart)
			{
				foreach (Item item in Main.item)
				{
					...
					{
						item.SetDefaults(ModContent.ItemType<GoldenHeart>());
					}
				}
			}
			if (healBonus > 0 && player.statLife < player.statLifeMax2 && Math.Abs(player.velocity.X) + Math.Abs(player.velocity.Y) < 1.0f)
			{
				...
				if ((int)timeToSpawn > 70 - magicalHealCooldown)
				{
					...
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<HeartFlame>(), 0, 0.0f, player.whoAmI, 0.0f, healBonus);
					timeToSpawn = 0.0f;
			...
			if (giftCooldown > 2)
			{
				if (rubyG)
				{
					...
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<RubyFlame>(), 0, 0.0f, player.whoAmI, 0.0f, 0.0f);
				}
				if (emeraldG)
				{
					Vector2 xyCenter = player.Center + new Vector2(Main.rand.Next(-300, 300), Main.rand.Next(-300, 300));
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<EmeraldFlame>(), 0, 0.0f, player.whoAmI, 0.0f, 0.0f);
				}
				if (amethystG)
				{
					Vector2 xyCenter = player.Center + new Vector2(Main.rand.Next(-300, 300), Main.rand.Next(-300, 300));
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<AmethystFlame>(), 0, 0.0f, player.whoAmI, 0.0f, 0.0f);
				}
				if (topazG)
				{
					Vector2 xyCenter = player.Center + new Vector2(Main.rand.Next(-300, 300), Main.rand.Next(-300, 300));
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<TopazFlame>(), 0, 0.0f, player.whoAmI, 0.0f, 0.0f);
				}
				if (diamondG)
				{
					Vector2 xyCenter = player.Center + new Vector2(Main.rand.Next(-300, 300), Main.rand.Next(-300, 300));
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<DiamondFlame>(), 0, 0.0f, player.whoAmI, 0.0f, 0.0f);
				}
				if (sapphireG)
				{
					Vector2 xyCenter = player.Center + new Vector2(Main.rand.Next(-300, 300), Main.rand.Next(-300, 300));
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<SapphireFlame>(), 0, 0.0f, player.whoAmI, 0.0f, 0.0f);
				}
				if (specterG)
				{
					Vector2 xyCenter = player.Center + new Vector2(Main.rand.Next(-400, 200), Main.rand.Next(-400, 200));
					Projectile.NewProjectile(xyCenter, Vector2.Zero, ModContent.ProjectileType<SpecterFlame>(), 0, 0.0f, player.whoAmI, 0.0f, 0.0f);
				}
				giftCooldown = 0;
			}
			if (starStone)
			{
				foreach (Item item in Main.item)
				{
					...
					{
						item.SetDefaults(ModContent.ItemType<PlatinumStar>());
					}
				}
			}
			if (player.inventory[player.selectedItem].useAmmo == ModContent.ItemType<Buckshot>() && (player.inventory[player.selectedItem].type == ModContent.ItemType<GoldHuntingBlunderbuss>() || player.inventory[player.selectedItem].type == ModContent.ItemType<PlatinumHuntingBlunderbuss>()))
			{
				player.scope = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<DwarfShark>())
			{
				player.scope = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<HallowedBlunderbuss>())
			{
				player.scope = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<DeadShot>())
			{
				player.scope = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<ThoriumBlunderbuss>())
			{
				player.scope = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<TrueHallowedBlunderbuss>())
			{
				player.scope = true;
			}
			...
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<PruningShears>())
			{
				player.cordage = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<GooFishingPole>())
			{
			...
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<Bonebardier>())
			{
				Bonebardier = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<SatBow>())
			{
				SatBow = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<SteelKnife>())
			{
				if (player.itemAnimation > 0)
				{
			...
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<BoilingPoint>())
			{
				boilingPoint = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<CherryBlossom>())
			{
				cherryBlossom = true;
			}
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<HighRoller>() || player.inventory[player.selectedItem].type == ModContent.ItemType<TrueHighRoller>())
			{
			...
			if (player.inventory[player.selectedItem].type == ModContent.ItemType<GigaVolt>())
			{
				if (MinionType == -1)
				{
					MinionType = ModContent.ProjectileType<VoltCharge>();
				}
			...
			if (player.inventory[player.selectedItem].melee && !player.inventory[player.selectedItem].noUseGraphic)
			{
				...
				if (!player.inventory[player.selectedItem].autoReuse && !player.noItems)
				{
					...
					{
						if (player.inventory[player.selectedItem].shoot > ProjectileID.None && player.whoAmI != Main.myPlayer && player.controlUseItem)
						{
							return;
						}
						else
			...
			if (player.inventory[player.selectedItem].type != ModContent.ItemType<OrcishAxe>())
			{
			...
			int crystal = player.FindItem(ModContent.ItemType<GlacialCrystal>());
			...
			if (player.inventory[player.selectedItem].type != ModContent.ItemType<HeavenlyClock>() && player.GetModPlayer<AntiarisPlayer>(mod).heavenWarn != 0)
			{
			...
			if (AntiarisWorld.heavenClock == 1)
			{
				...
				if (AntiarisWorld.heavenTimer == duration - 1)
				{
					Main.LocalPlayernventory = false;
				...
				try
				{
					...
					{
						SoundEngine.PlaySound(mod.GetLegacySoundSlot(Terraria.ModLoader.SoundType.Custom, "Sounds/Items/Singularity"), player.position);
			...
			for (int i = 3; i < 8 + player.extraAccessorySlots; i++)
			{
				...
				{
					player.npcTypeNoAggro[ModContent.NPCType<HoneySlime>()] = true;
					player.npcTypeNoAggro[ModContent.NPCType<CrispyHoneySlime>()] = true;
				}
			}
			if (beePheromones)
			{
		...
		public override void ModifyHitByNPC(NPC npc, ref int damage, ref bool crit)
		{
			if (RuneofWrath)
			{
				player.AddBuff(ModContent.BuffType<Wrath>(), 300);
			}
			if (RuneofTranquility)
			{
				player.AddBuff(ModContent.BuffType<Tranquility>(), 300);
			}
			if (RuneofBleeding)
			{
				for (int k = 0; k < 200; k++)
				{
					if (Main.npc[k].lifeMax > 0 && Main.npc[k].active && !Main.npc[k].friendly && Main.npc[k].damage > 0 && !Main.npc[k].dontTakeDamage)
					{
						...
						Main.npc[k].StrikeNPC(25, 1f, direction, Main.rand.Next(2) == 0 ? true : false, false, false);
						if (Main.netMode != NetmodeID.SinglePlayer)
							NetMessage.SendData(MessageID.StrikeNPC, -1, -1, NetworkText.FromLiteral(""), Main.npc[k].whoAmI, 1, 1f, direction, 25);
					}
				}
			}
		}
		...
		public override void OnHitNPC(Item item, NPC target, int damage, float knockback, bool crit)
		{
			...
			if (mRing && distanceTo <= (double)distance && target.life <= 0)
			{
				int newMana = (int)(player.statManaMax2 * 0.15f);
				player.statMana += newMana;
				player.ManaEffect(newMana);
			}
			if (hRing && distanceTo <= (double)distance && target.life <= 0)
			{
		...
		public override void OnHitNPCWithProj(Projectile proj, NPC target, int damage, float knockback, bool crit)
		{
			...
			if (discipleSet && proj.magic)
			{
				...
				if (targetHit >= 3)
				{
					...
					SoundEngine.PlaySound(2, (int)proj.position.X, (int)proj.position.Y, 8);
					Projectile.NewProjectile(proj.position.X, proj.position.Y, 0.0f, 0.0f, ModContent.ProjectileType<ManaEffect>(), 0, 0.0f, owner, manaAmount, 0.0f);
					targetHit = 0;
				}
			}
		...
		public override void OnHitAnything(float x, float y, Entity victim)
		{
			...
			if (enchantedSet == true && Main.rand.Next(3) == 0)
			{
				...
				Projectile.NewProjectile(DistX, DistY, direction.X + A, direction.Y + B, ModContent.ProjectileType<EnchantedDagger>(), 15, 1, player.whoAmI, 0f, 0f);
			}
		}
		...
		public override void OnHitByNPC(NPC npc, int damage, bool crit)
		{
			...
			int HarpyEgg = player.FindItem(ModContent.ItemType<HarpyEgg>());
			if (HarpyEgg != -1)
			{
				if (Main.rand.Next(4) == 0)
				{
					player.inventory[HarpyEgg].stack = 0;
					SoundEngine.PlaySound(3, (int)player.position.X, (int)player.position.Y, 1);
			...
			if (honeyBadger)
			{
				...
				{
					if (player.buffType[j] == ModContent.BuffType<BadgersResolve>())
						player.DelBuff(j);
				}
				if (!player.buffType.Contains(ModContent.BuffType<HoneyRecharge>()))
					player.AddBuff(ModContent.BuffType<BadgersResolve>(), 300);
			}
			if (mellifiedSkull)
			{
		...
		public override bool PreHurt(bool pvp, bool quiet, ref int damage, ref int hitDirection, ref bool crit, ref bool customDamage, ref bool playSound, ref bool genGore, ref PlayerDeathReason damageSource)
		{
			...
			{
				playSound = false;
				SoundEngine.PlaySound(SoundID.NPCHit31, player.position);
			}
			if (AntiarisWorld.heavenClock == 1)
			{
				return false;
		...
		{
			QuestSystem questSystem = player.GetModPlayer<QuestSystem>();
			...
			{
				caughtType = ModContent.ItemType<OldCompass>();
			}
			if ((PlayerPosition < 380 || PlayerPosition > Main.maxTilesX - 380) && Main.rand.Next(22) == 0)
			{
				caughtType = ModContent.ItemType<Pufferfish>();
			}
			if (gooFishingPole && Main.rand.Next(20) == 0)
			{
				caughtType = ModContent.ItemType<SlimyCrate>();
			}
		}
	}
}
