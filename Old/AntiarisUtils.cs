...
namespace Antiaris
{
	public static class AntiarisUtils
	{
		...
		{
			bool flag = false;
			int i = (int)(Main.mouseX + (double)Main.screenPosition.X) / 16;
			int j = (int)(Main.mouseY + (double)Main.screenPosition.Y) / 16;
			if (player.gravDir == -1.0)
				j = (int)(Main.screenPosition.Y + (double)Main.screenHeight - Main.mouseY) / 16;
			worldX = i * 16 + 8;
			pushYUp = 41;
			if (!flag)
			{
		...
		{
			System.Collections.Generic.IEnumerable<int> something = Enumerable.Range(1, 10);
		}
	}
}
