...
namespace Antiaris.Tiles.Decorations
{
	public class TheKiller : ModTile
	{
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			if (frameX == 0)
			{
				Item.NewItem(i * 16, j * 16, 48, 48, ModContent.ItemType<TheKiller>(), 1, false, 0, false, false);
			}
		}
		...
		public override void NearbyEffects(int i, int j, bool closer)
		{
			...
			if (distanceTo <= (double)distance && Main.bloodMoon && !player.GetModPlayer<AntiarisPlayer>(mod).bitesTheDust)
			{
				player.AddBuff(ModContent.BuffType<BitesTheDust>(), 600, true);
			}
		}
	}
}