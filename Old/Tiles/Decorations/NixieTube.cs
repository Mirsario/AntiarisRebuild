...
namespace Antiaris.Tiles.Decorations
{
	public class NixieTube : ModTile
	{
		...
		public override void SetDefaults()
		{
			...
			name.AddTranslation((int)GameCulture.CultureName.Russian, "Газоразрядный индикатор");
		...
		{
			Item.NewItem(i * 16, j * 16, 16, 48, ModContent.ItemType<NixieTube>(), 1, false, 0, false, false);
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			if (entity != null)
			{
				...
				if (entity.Chip.type > ItemID.None)
		...
		public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			if (entity != null)
			{
				if (entity.Lightbulb.type <= ItemID.None)
					Main.tileTexture[Type] = mod.GetTexture("Tiles/Decorations/NixieTube");
		...
		public override bool RightClick(int i, int j)
		{
			...
			SoundEngine.PlaySound(10, (int)player.position.X, (int)player.position.Y, 0);

			NixieTubeUI.assignedTile = this;

			NixieTubeUI.entity = entity;

			if (!(NixieTubeUI.entity.Lightbulb.modItem is Lightbulb2))
				NixieTubeUI.entity.Lightbulb = new Item();
			if (!(NixieTubeUI.entity.Chip.modItem is LightingChip))
		...
		Point16 EntityPosition(int i, int j)
		{
			return new Point16(i - Main.tile[i, j].frameX % 36 / 18 + 1, j - Main.tile[i, j].frameY % 56 / 18 + 1);
		}
