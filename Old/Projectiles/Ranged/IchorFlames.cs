...
namespace Antiaris.Projectiles.Ranged
{
	public class IchorFlames : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "脓血烈焰");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Ихоровое пламя");
		}
		...
		public override void AI()
		{
			...
			if (projectile.ai[0] > 6f)  //this defines where the flames starts
			{
				if (Main.rand.Next(2) == 0)  //this defines how many dust to spawn
				{
					...
					Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 159, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, 130, default, 1.5f);
				}
			}
			else
			{
