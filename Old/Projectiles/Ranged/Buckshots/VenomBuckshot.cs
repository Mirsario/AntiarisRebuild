...
{
	public class VenomBuckshot : ModProjectile
	{
		...
		{
			for (int k = 0; k < 8; k++)
				Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 70, projectile.oldVelocity.X * 0.2f, projectile.oldVelocity.Y * 0.2f);
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			target.AddBuff(ModContent.BuffType<Deceleration>(), 120);
