...
namespace Antiaris.Projectiles.Ranged.Buckshots
{
	public class CursedBuckshot : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "咒怨火铳弹");
		...
		public override void AI()
		{
			...
			{
				if (!npc.friendly && npc.active && Vector2.Distance(npc.Center, projectile.Center) <= 100.0)
				{
					if (timer == 30)
					{
						npc.StrikeNPC(projectile.damage / 5, 1f, 1, Main.rand.Next(2) == 0 ? true : false, false, false);
						if (Main.netMode != NetmodeID.SinglePlayer)
							NetMessage.SendData(MessageID.StrikeNPC, -1, -1, NetworkText.FromLiteral(""), npc.whoAmI, 1, 1f, 1, projectile.damage / 5);
						timer = 0;
					}
				}
			}
