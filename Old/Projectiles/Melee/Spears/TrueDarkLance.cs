...
namespace Antiaris.Projectiles.Melee.Spears
{
	public class TrueDarkLance : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Истинная Темная Пика");
		}
		...
		public override void AI()
		{
			...
			if (player.itemAnimation < player.itemAnimationMax / 3)
			{
				...
				if (projectile.localAI[0] == 0f)
				{
					...
					if (player.itemAnimation < player.itemAnimationMax / 2)
						Projectile.NewProjectile(projectile.Center.X + projectile.velocity.X, projectile.Center.Y + projectile.velocity.Y, projectile.velocity.X * 1.5f, projectile.velocity.Y * 1.5f, ModContent.ProjectileType<TrueDarkLancePro>(), projectile.damage, projectile.knockBack * 0.85f, projectile.owner, 0f, 0f);
				}
			}
			...
			{
				int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 205, projectile.velocity.X * 0.2f + projectile.direction * 3, projectile.velocity.Y * 0.2f, 100, new Color(), 1.2f);
				Main.dust[dust].noGravity = true;
