...
{
	public class RubyFlame : ModProjectile
	{
		...
		{
			DisplayName.SetDefault("Ruby Flame");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Рубиновое пламя");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "红宝石火焰");
		...
		public override void AI()
		{
			...
			{
				player.AddBuff(ModContent.BuffType<RubyEssence>(), 300, true);
				projectile.Kill();
			}
			float ai2 = 0.5f;
			...
			for (float k = 0.0f; k < 3.0; k++)
				...
				{
					Dust dust = Main.dust[Dust.NewDust(projectile.Center, 0, 0, 60, 0.0f, -2.0f, 0, new Color(), 1.0f)];
