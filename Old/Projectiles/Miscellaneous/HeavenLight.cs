...
namespace Antiaris.Projectiles.Miscellaneous
{
	public class HeavenLight : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (num608 < 50f && projectile.position.X < Main.player[num604].position.X + Main.player[num604].width && projectile.position.X + projectile.width > Main.player[num604].position.X && projectile.position.Y < Main.player[num604].position.Y + Main.player[num604].height && projectile.position.Y + projectile.height > Main.player[num604].position.Y)
			{
				projectile.Kill();
			}
			...
			{
				float num615 = projectile.velocity.X * 0.2f * num614;
				float num616 = -(projectile.velocity.Y * 0.2f) * num614;
				int num617 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 64, 0f, 0f, 100, default, 1.1f);
