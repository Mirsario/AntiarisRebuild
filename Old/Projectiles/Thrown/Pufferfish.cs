...
namespace Antiaris.Projectiles.Thrown
{
	public class Pufferfish : ModProjectile
	{
		...
		{
			SoundEngine.PlaySound(mod.GetLegacySoundSlot(Terraria.ModLoader.SoundType.Custom, "Sounds/Projectiles/Pufferfish"), projectile.position);
		}
		...
		public override void AI()
		{
			...
			if (isStickingToTarget)
			{
				...
				else if (Main.npc[projTargetIndex].active && !Main.npc[projTargetIndex].dontTakeDamage)
				{
					...
					if (hitEffect)
					{
						int direction = Main.npc[projTargetIndex].direction;
						...
						Main.npc[projTargetIndex].StrikeNPC(11, 1f, direction, false, false, false);
						if (Main.netMode != NetmodeID.SinglePlayer)
							NetMessage.SendData(MessageID.StrikeNPC, -1, -1, NetworkText.FromLiteral(""), Main.npc[projTargetIndex].whoAmI, 1, 1f, direction, 11);
					}
				}
				else
				{
		...
		public override void Kill(int timeLeft)
		{
			...
			if (Main.tile[(int)projectile.position.X / 16, (int)projectile.position.Y / 16].liquid == 0 || Main.tile[(int)projectile.position.X / 16, (int)projectile.position.Y / 16].liquidType() == 0)
			{
			...
			if (projectile.owner == Main.myPlayer)
			{
				...
				for (int i = 0; i < amount; i++)
				{
					...
					value17.Normalize();
					value17 *= 100 * 0.03f;
					int bubble = Projectile.NewProjectile(projectile.position.X, projectile.position.Y, value17.X, value17.Y, ModContent.ProjectileType<Bubble>(), 10, 1f, Main.myPlayer, 0f);
				...
				int fish = Projectile.NewProjectile(pos.X, pos.Y, 0, 0, ModContent.ProjectileType<SmallPufferfish>(), 0, 0f, player.whoAmI, 0f);
