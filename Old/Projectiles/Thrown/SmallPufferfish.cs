		...
		public override void Kill(int timeLeft)
		{
			if (projectile.owner == Main.myPlayer)
			{
				...
				else
				{
					...
					int puffer = Item.NewItem((int)projectile.position.X, (int)projectile.position.Y, projectile.width, projectile.height, ModContent.ItemType<Pufferfish>());

					if (Main.netMode == NetmodeID.MultiplayerClient && puffer >= 0)
					{
