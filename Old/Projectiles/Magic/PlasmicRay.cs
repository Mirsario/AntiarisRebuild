...
namespace Antiaris.Projectiles.Magic
{
	public class PlasmicRay : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Плазменный луч");
		}
			...
			if (timer > 9f)
			{
				if (timer == 10f)
				{
					...
					for (int k = 0; k < (double)count; k++)
					{
						...
						int dust = Dust.NewDust(projectile.Center - new Vector2(0.0f, 4.0f), 1, 1, ModContent.DustType<BlueLaser>(), 0f, 0f, 200, Scale: 1.55f);
				...
				for (int k = 0; k < 1; k++)
				{
					...
					var dust = Dust.NewDustDirect(projectile.position, 1, 1, ModContent.DustType<BlueLaser>(), 0f, 0f, 200, Scale: 1.55f);
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			{
				int sphere = Projectile.NewProjectile(target.position, new Vector2(0f, 0f), ProjectileID.Electrosphere, projectile.damage, 0, player.whoAmI);
