...
namespace Antiaris.Projectiles.Magic
{
	public class CurseSphere : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Сфера проклятия");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "诅咒火球");
		...
		public override void Kill(int timeLeft)
		{
			for (int k = 0; k < 7; k++)
			{
				int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 107, Main.rand.Next(-4, 4), Main.rand.Next(-4, 4), 0, new Color(), 1.5f);
				Main.dust[dust].noGravity = true;
			}
		}
	}
