...
namespace Antiaris.Projectiles.Magic
{
	public class Lightning : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "真正的落雷");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Молния");
		}
		...
		{
			if (Vector2.Distance(Main.player[projectile.owner].Center, projectile.Center) > 1000.0)
				projectile.Kill();
			if (projectile.velocity == Vector2.Zero)
			{
				float num1 = (float)(projectile.rotation + 1.57079637050629 + (Main.rand.Next(2) == 1 ? -1.0 : 1.0) * 1.57079637050629);
			...
			if (projectile.timeLeft >= 596)
				return;
			if (projectile.localAI[1] == 0.0 && projectile.ai[0] >= 900.0)
			{
			...
			if (projectile.velocity == Vector2.Zero)
			{
				...
				if (Main.rand.Next(projectile.extraUpdates) != 0 || !(projectile.velocity != Vector2.Zero) && Main.rand.Next(projectile.localAI[1] == 2.0 ? 2 : 6) != 0)
					return;
				for (int k = 0; k < 2; k++)
				{
				...
				if (Main.rand.Next(5) != 0)
					return;
				int index3 = Dust.NewDust(projectile.Center + projectile.velocity.RotatedBy(1.57079637050629, new Vector2()) * ((float)Main.rand.NextDouble() - 0.5f) * projectile.width - Vector2.One * 4f, 8, 8, 31, 0.0f, 0.0f, 100, new Color(), 1.5f);
			...
			else
			{
				...
				do
				{
					...
					if (rotationVector2.Y > -0.0199999995529652)
						flag = true;
					if (rotationVector2.X * (double)(projectile.extraUpdates + 1) * 2.0 * num1 + projectile.localAI[0] > 25.0)
						flag = true;
					if (rotationVector2.X * (double)(projectile.extraUpdates + 1) * 2.0 * num1 + projectile.localAI[0] < -25.0)
						flag = true;
					if (flag)
			...
			label_37:
				...
				if (Main.rand.Next(5) != 0 || Main.netMode == NetmodeID.MultiplayerClient || projectile.localAI[1] != 0.0)
					return;
				Vector2 v = projectile.ai[0].ToRotationVector2().RotatedBy(Main.rand.Next(-1, 1) * 1.04719758033752 / 3.0, new Vector2()) * projectile.velocity.Length();
