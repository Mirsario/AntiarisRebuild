...
namespace Antiaris.Projectiles.Magic
{
	public class CopyOrderEnergy : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "秩序之力");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Энергия порядка");
		...
		public override void AI()
		{
			...
			for (int k = 0; k < 200; ++k)
			{
				...
				if (npc.active && !npc.dontTakeDamage && !npc.friendly && npc.lifeMax > 5 && npc.type != NPCID.TargetDummy && projectile.Distance(npc.Center) < (double)distance && Collision.CanHit(projectile.Center, 1, 1, npc.Center, 1, 1))
				{
					...
					{
						if (npc.position.X > (double)projectile.position.X)
							projectile.direction = 1;
						else if (npc.position.X < (double)projectile.position.X)
							projectile.direction = -1;
						distance = distanceTo;
						moveX = moveToX;
						moveY = moveToY;
			...
			try
			{
				...
				this.UpdateBolt();
				foreach (Projectile projectile_ in Main.projectile)
					if (Vector2.Distance(projectile.Center, projectile_.Center) < 200.0 && projectile_.active && projectile_.ai[0] == projectile.ai[0] && this.bolt.Ticks <= 0 && Main.rand.NextBool(1, 2))
					{
		...
		public override void Kill(int timeLeft)
		{
			for (int k = 0; k < 13; k++)
			{
				int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, ModContent.DustType<Order>(), 0.0f, 0.0f, 100, new Color(), 1f);
