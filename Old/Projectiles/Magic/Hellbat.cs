...
namespace Antiaris.Projectiles.Magic
{
	public class Hellbat : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "地狱蝙蝠");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Адская летучая мышь");
		...
		{
			this.SetTag(ProjectileTags.Incendiary);
			this.SetTag(ProjectileTags.IncendiarySlow);
		}*/
		...
		public override void AI()
		{
			...
			{
				NPC npc = Main.npc[k];
				if (!npc.wet && npc.active && !npc.dontTakeDamage && !npc.friendly && npc.lifeMax > 5 && npc.type != NPCID.TargetDummy && projectile.Distance(npc.Center) < (double)distance && Collision.CanHit(projectile.Center, 1, 1, npc.Center, 1, 1))
				{
