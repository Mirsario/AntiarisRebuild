...
namespace Antiaris.Projectiles.Magic
{
	public class RubyRoseBolt : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "卢比箭");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Снаряд рубиновой розы");
		}
		...
		public override void AI()
		{
			for (int i = 0; i < 10; i++)
			{
				...
				int dust = Dust.NewDust(new Vector2(x, y), 1, 1, 60, 0f, 0f, 0, default, 2f);
		...
		public override bool OnTileCollide(Vector2 oldVelocity)
		{
			...
			else
			{
				...
				if (projectile.velocity.X != (double)oldVelocity.X)
				{
					projectile.velocity.X = -oldVelocity.X;
				}
				if (projectile.velocity.Y != (double)oldVelocity.Y)
				{
					projectile.velocity.Y = -oldVelocity.Y;
				}
				SoundEngine.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y, 10, 1f, 0.0f);
				for (int k = 0; k < 15; ++k)
				{
