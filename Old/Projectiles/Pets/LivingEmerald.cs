...
namespace Antiaris.Projectiles.Pets
{
	public class LivingEmerald : ModProjectile
	{
		...
		public override void AI()
		{
			...
			Player player = owner;
			//from vanilla
			bool flag = projectile.type == ModContent.ProjectileType<LivingEmerald>();
			...
			{
				int index = Dust.NewDust(projectile.position - projectile.velocity, projectile.width, projectile.height, 61, 0.0f, 0.0f, 0, new Color(), 1f);
			...
			else
			{
				...
				if (flag1)
				{
					vector2_1.X -= (15 + player.width / 2) * player.direction;
					vector2_1.X -= projectile.minionPos * 40 * player.direction;
				}
				if (projectile.type == ModContent.ProjectileType<LivingEmerald>())
				{
					vector2_1.X -= (15 + player.width / 2) * player.direction;
					vector2_1.X -= 40 * player.direction;
				}
				bool flag2 = true;
				if (projectile.type == ModContent.ProjectileType<LivingEmerald>() || projectile.type == 653)
					flag2 = false;
				int index1 = -1;
				float num1 = 450f;
				if (flag1)
				...
				int num2 = 15;
				Vector2 vector2_2;
				if (projectile.ai[0] == 0.0 && flag2)
				{
					NPC minionAttackTargetNpc = projectile.OwnerMinionAttackTargetNPC;
					if (minionAttackTargetNpc != null && minionAttackTargetNpc.CanBeChasedBy(projectile, false))
					{
				...
				if (projectile.ai[0] == 1.0)
				{
					...
					float num4 = 10f;
					int num5 = 200;
					if (num4 < Math.Abs(player.velocity.X) + (double)Math.Abs(player.velocity.Y))
					...
					if (num6 < (double)num5 && player.velocity.Y == 0.0 && (projectile.position.Y + (double)projectile.height <= player.position.Y + (double)player.height && !Collision.SolidCollision(projectile.position, projectile.width, projectile.height)))
					{
						projectile.ai[0] = 0.0f;
						projectile.netUpdate = true;
						if (projectile.velocity.Y < -6.0)
					...
					if (num6 >= 60.0)
					{
						vector2_3.Normalize();
						vector2_3 *= num4;
						if (projectile.velocity.X < (double)vector2_3.X)
						{
						...
						if (projectile.velocity.Y < (double)vector2_3.Y)
						{
				...
				if (projectile.ai[0] == 2.0)
				{
					...
					projectile.frame = 4 + (int)(num2 - (double)projectile.ai[1]) / (num2 / 3);
					if (projectile.velocity.Y != 0.0)
						projectile.frame += 3;
				...
				if (index1 >= 0)
				{
					...
					{
						vector2_1 = center;
						if (center.Y < projectile.Center.Y - 30.0 && projectile.velocity.Y == 0.0)
						{
				...
				if (projectile.ai[0] == 0.0 && index1 < 0)
				{
					float num3 = 500f;
					if (projectile.type == ModContent.ProjectileType<LivingEmerald>())
						num3 = 200f;
					...
					Vector2 vector2_3 = player.Center - projectile.Center;
					if (vector2_3.Length() > 2000.0)
						projectile.position = player.Center - new Vector2(projectile.width, projectile.height) / 2f;
					else if (vector2_3.Length() > (double)num3 || Math.Abs(vector2_3.Y) > 300.0)
					{
						projectile.ai[0] = 1f;
						projectile.netUpdate = true;
						if (projectile.velocity.Y > 0.0 && vector2_3.Y < 0.0)
				...
				if (projectile.ai[0] == 0.0)
				{
					...
					float num5 = 4f;
					float num6 = 0.1f;
					if (num5 < Math.Abs(player.velocity.X) + (double)Math.Abs(player.velocity.Y))
					{
					...
					if (Math.Abs(num8) > 5.0)
					{
						if (num8 < 0.0)
						{
							num7 = -1;
							if (projectile.velocity.X > -(double)num4)
								projectile.velocity.X -= num3;
							else
					...
					else
					{
						...
						if (Math.Abs(projectile.velocity.X) < num3 * 2.0)
							projectile.velocity.X = 0.0f;
					}
					if (num7 != 0)
					{
						int num9 = (int)(projectile.position.X + (double)(projectile.width / 2)) / 16;
					...
					if (projectile.type == ModContent.ProjectileType<LivingEmerald>() && projectile.velocity.X != 0.0)
						flag3 = true;
					if (projectile.type == 653 && projectile.velocity.X != 0.0)
						flag3 = true;
					...
					if (projectile.velocity.Y == 0.0 && flag3)
					{
						for (int index2 = 0; index2 < 3; ++index2)
						{
							int i1 = (int)(projectile.position.X + (double)(projectile.width / 2)) / 16;
							if (index2 == 0)
								i1 = (int)projectile.position.X / 16;
							if (index2 == 2)
								i1 = (int)(projectile.position.X + (double)projectile.width) / 16;
							int j = (int)(projectile.position.Y + (double)projectile.height) / 16;
							if (!WorldGen.SolidTile(i1, j))
							{
								if (!Main.tile[i1, j].halfBrick())
								{
									if (Main.tile[i1, j].slope() <= 0)
									{
										if (TileID.Sets.Platforms[Main.tile[i1, j].type] && Main.tile[i1, j].active())
										{
							...
							{
								int num9 = (int)(projectile.position.X + (double)(projectile.width / 2)) / 16;
								int num10 = (int)(projectile.position.Y + (double)(projectile.height / 2)) / 16;
					...
					if (projectile.velocity.X > (double)num5)
					...
					if (projectile.velocity.X > 0.0)
						projectile.direction = 1;
					if (projectile.velocity.X > (double)num3 && num7 == 1)
						projectile.direction = 1;
					if (projectile.velocity.X < -(double)num3 && num7 == -1)
						projectile.direction = -1;
					projectile.spriteDirection = projectile.direction;
