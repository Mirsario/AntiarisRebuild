...
namespace Antiaris.Projectiles.Pets
{
	public class ShadowflameCandle : ModProjectile
	{
		...
		public override void AI()
		{
			...
			float speed = 4.0f;
			Vector2 position = new Vector2(player.direction * 30, -20f);
			position.Y += (float)Math.Cos(projectile.ai[0] * 0.05) * 4.0f;
			Vector2 dustPos = new Vector2(projectile.spriteDirection == -1 ? -6.0f : -3.0f, -20.0f).RotatedBy(projectile.rotation, new Vector2());
			if (Main.rand.Next(24) == 0)
			{
				...
				dust.scale += (float)(0.1 + Main.rand.NextFloat() * 0.6);
			}
			projectile.rotation = projectile.velocity.X * 0.095f;
			projectile.spriteDirection = projectile.direction;
			if (player.controlDown && player.velocity == Vector2.Zero && player.velocity.Y == 0.0)
			{
				Vector2 vector = Main.MouseWorld;
				if (projectile.position.X > (double)vector.X)
					projectile.direction = -1;
				else if (projectile.position.X < (double)vector.X)
					projectile.direction = 1;
				if (Main.myPlayer != projectile.owner)
					return;
				float speed2 = 2.94f;
				Vector2 vector2 = vector - projectile.Center;
				float distance2 = (float)Math.Sqrt(vector2.X * (double)vector2.X + vector2.Y * (double)vector2.Y);
				vector2 *= speed2 / distance2;
				projectile.velocity = vector2;
			}
			else
			{
				if (player.suspiciouslookingTentacle || player.petFlagDD2Ghost)
					position.X += -player.direction * 64;
				...
				{
					if (zeroPoint.Length() < speed * 0.5)
						projectile.velocity = zeroPoint;
					else
						projectile.velocity = zeroPoint * 0.1f;
				}
			}
			++projectile.ai[0];
