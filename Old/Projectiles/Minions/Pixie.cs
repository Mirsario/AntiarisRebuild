...
namespace Antiaris.Projectiles.Minions
{
	public class Pixie : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Пикси");
		}
		...
		public override void AI()
		{
			...
			projectile.spriteDirection = projectile.velocity.X <= 0.0 ? 1 : -1;
			projectile.rotation = projectile.velocity.X * 0.1f;
			if (Main.rand.Next(6) == 0)
			{
				int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 173, 0.0f, 0.0f, 200, default, 1.0f);
				Main.dust[dust].velocity *= 0.3f;
			}
			if (Main.rand.Next(40) == 0)
				SoundEngine.PlaySound(27, (int)projectile.position.X, (int)projectile.position.Y, 1);
			if (projectile.tileCollide && WorldGen.SolidTile(Framing.GetTileSafely((int)projectile.Center.X / 16, (int)projectile.Center.Y / 16)))
				projectile.tileCollide = false;
			++projectile.ai[0];
			Vector2 Position2;
			if (projectile.ai[0] % 40.0 == 0.0)
			{
		...
		public override void Kill(int timeLeft)
		{
			for (int k = 0; k < 50; k++)
			{
				int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 173, 0.0f, -2.0f, 0, new Color(), 2.0f);
