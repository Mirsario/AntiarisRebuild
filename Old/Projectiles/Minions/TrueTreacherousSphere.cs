...
namespace Antiaris.Projectiles.Minions
{
	public class TrueTreacherousSphere : ModProjectile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Истинная коварная сфера");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "千瞬光球");
		...
		public override void AI()
		{
			...
			if (minionAttackTargetNpc != null && minionAttackTargetNpc.CanBeChasedBy(projectile, true))
			{
			...
			if (target)
			{
				...
				if (aiTimer >= 1.0)
					this.aiTimer = 0.0f;
				if (projectile.soundDelay <= 0 && chargedTimer > 0.0 && projectile.ai[0] <= 0.0)
				{
					projectile.soundDelay = delay;
					projectile.soundDelay *= 2;
					if (aiTimer != 1.0)
						SoundEngine.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y, 75);
				}
			}
			if (Main.myPlayer != projectile.owner)
				return;
			if (chargedTimer % 5.0 == 1.0 && chargedTimer < 100.0 && projectile.ai[1] != 0.0)
			{
				float speed = (float)(3.0 + Main.rand.NextFloat() * 6.0);
				Vector2 start = Vector2.UnitY.RotatedByRandom(6.28318548202515);
				int j = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, start.X * speed, start.Y * speed, ModContent.ProjectileType<TrueTreacherousEnergy>(), projectile.damage, projectile.knockBack * 2.0f, projectile.owner, Main.npc[targetNPC].whoAmI, 0.0f);
				Main.projectile[j].minion = true;
				Main.projectile[j].magic = false;
			}
			if (chargedTimer >= 100.0)
			{
				projectile.ai[0] = 180.0f;
				this.chargedTimer = 0;
			}
			if (projectile.ai[0] % 30.0 == 0.0 && projectile.ai[0] > 0.0f)
			{
				for (int j = 0; j < 50; j++)
				{
					...
					if (Main.dust[dust].position != projectile.Center)
						Main.dust[dust].velocity = projectile.DirectionTo(Main.dust[dust].position) * 2.0f;
				}
			}
		...
		public override void Kill(int timeLeft)
		{
			for (int k = 0; k < 13; k++)
			{
				...
				Main.dust[dust].velocity = velocity.RotatedBy(15 * (k + 2), new Vector2());
			}
		}
	}
}
