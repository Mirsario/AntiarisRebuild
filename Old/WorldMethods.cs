...
namespace Antiaris
{
	...
	public class WorldMethods
	{
		public static void RoundHill(int X, int Y, int Xmult, int Ymult, int strength, bool initialplace, ushort type)
		{
			if (initialplace)
			{
				WorldMethods.TileRunner(X, Y, strength, 1, type, true, 0f, 0f, true, true);
			}
			int rotation2 = 0;
			for (rotation2 = 0; rotation2 < 350; rotation2++)
			{
				int DistX = (int)(0 - (Math.Sin(rotation2) * Xmult));
				int DistY = (int)(0 - (Math.Cos(rotation2) * Ymult));
				WorldMethods.TileRunner(X + DistX, Y + DistY, strength, 1, type, true, 0f, 0f, true, true);
			}
		}

		public static void MainVolcano(int X, int Y, int length, int height, ushort type2, float slope, float sloperight, ushort type3)
		{
			...
			for (int level = 0; level <= height; level++)
			{
				...
				for (int I = X - (int)((length + (level * trueslope)) + (Math.Sqrt(level) * 3)); I < X + (int)((length + (level * truesloperight)) + (Math.Sqrt(level) * 3)); I++)
				{
					...
					//	{
					if (level < 17 && Main.rand.Next(16 - level) < 4)
					{
						WorldGen.KillWall(I, Y + level);
						WorldGen.PlaceWall(I, Y + level, type3);
					}
					if (level >= 16)
					{
					...
					{
						WorldMethods.CragSpike(I, (Y + level) - Main.rand.Next(5, 15), 1, 30, type2, Main.rand.Next(2, 6), Main.rand.Next(2, 6));
					}
					//}
				}
			}
		...
		public static void RoundHole(int X, int Y, int Xmult, int Ymult, int strength, bool initialdig)
		{
			...
			for (int rotation2 = 0; rotation2 < 350; rotation2++)
			{
				...
				{
					WorldGen.digTunnel(X + (DistX * 3.5f), Y + (DistY * 3.5f), 0, 0, Main.rand.Next(4, 7), Main.rand.Next(4, 7), false);
					for (int l = 0; l < 20; l++)
					{
						int x1 = Main.rand.Next(-10, 10);
						int y1 = Main.rand.Next(-10, 10);
						WorldGen.PlaceObject((int)(X + (DistX * 2.9f)) + x1, (int)(Y + (DistY * 3.25f)) + y1, 31);
						NetMessage.SendObjectPlacment(-1, (int)(X + (DistX * 3.25f)) + x1, (int)(Y + (DistY * 3.25f)) + y1, 31, 0, 0, -1, -1);
					}
				}
			}
		...
		public static void TileRunner(int i, int j, double strength, int steps, int type, bool addTile = false, float speedX = 0f, float speedY = 0f, bool noYChange = false, bool overRide = true)
		{
			...
			pos.Y = j;
			Vector2 randVect;
			randVect.X = WorldGen.genRand.Next(-10, 11) * 0.1f;
			randVect.Y = WorldGen.genRand.Next(-10, 11) * 0.1f;
			...
			while (num > 0.0 && num2 > 0f)
			{
				...
				num2 -= 1f;
				int num3 = (int)(pos.X - num * 0.5);
				int num4 = (int)(pos.X + num * 0.5);
				int num5 = (int)(pos.Y - num * 0.5);
				int num6 = (int)(pos.Y + num * 0.5);
				if (num3 < 1)
				{
					num3 = 1;
				}
				...
				for (int k = num3; k < num4; k++)
				{
					...
					{
						if (Math.Abs(k - pos.X) + Math.Abs(l - pos.Y) < strength * 0.5 * (1.0 + WorldGen.genRand.Next(-10, 11) * 0.015))
						{
				...
				if (num > 50.0)
				{
					pos += randVect;
					num2 -= 1f;
					randVect.Y += WorldGen.genRand.Next(-10, 11) * 0.05f;
					randVect.X += WorldGen.genRand.Next(-10, 11) * 0.05f;
					if (num > 100.0)
					{
						pos += randVect;
						num2 -= 1f;
				...
				if (type == 59 && !noYChange)
				{
					...
					if (pos.Y < Main.rockLayer + 100.0)
					{
						randVect.Y = 1f;
					}
					if (pos.Y > Main.maxTilesY - 300)
					{
						randVect.Y = -1f;
					}
				}
		...
		public static void makeTemple(int x, int y)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				while (flag)
				{
					...
					if (i == num2 - 1)
					{
						...
						num14 = (int)((float)num14 * 1.6);
						num15 = (int)((float)num15 * 1.35);
			...
			for (int num41 = 0; num41 < num2; num41++)
			{
				...
				while (flag2)
				{
					...
					if (vector.X == num42 && vector.Y == num43)
					{
						flag2 = false;
					}
				}
				if (num41 < num2 - 1)
				{
					if (Main.rand.Next(3) != 0)
					{
						...
						if (array[num44].Y >= array[num41].Y + array[num41].Height)
						{
							...
							{
								rectangle2.X += (int)((float)array[num44].Width * 0.2);
							}
							else
							{
								rectangle2.X += (int)((float)array[num44].Width * 0.8);
							}
						...
						else
						{
							...
							rectangle2.Y = (int)(array[num44].Y + array[num44].Height * 0.8);
						}
						int x2 = rectangle2.X;
						int y3 = rectangle2.Y;
						flag2 = true;
						while (flag2)
						{
							...
							if (vector.X == num45 && vector.Y == num46)
							{
								flag2 = false;
							}
						}
					...
					else
					{
						...
						while (flag2)
						{
							...
							if (vector.X == num50 && vector.Y == num51)
							{
								flag2 = false;
							}
						}
		...
		IL_1547:
			float num96 = num2 * 1.1f;
			num96 *= 1f + Main.rand.Next(-25, 26) * 0.01f;
			int num97 = 0;
			while (num96 > 0f)
			{
				num97++;
		...
		public static void templePart2()
		{
			...
			num3 *= 1f + Main.rand.Next(-15, 16) * 0.01f;
			int num4 = 0;
			while (num3 > 0f)
			{
			...
			float num7 = num2 * 0.35f;
			num7 *= 1f + Main.rand.Next(-15, 16) * 0.01f;
			int contain = 1293;
			num4 = 0;
			while (num7 > 0f)
			{
			...
			float num10 = num2 * 1.25f;
			num10 *= 1f + Main.rand.Next(-25, 36) * 0.01f;
			num4 = 0;
			while (num10 > 0f)
			{
				num4++;
			...
			float num15 = num2 * 1.35f;
			num15 *= 1f + Main.rand.Next(-15, 26) * 0.01f;
			num4 = 0;
			while (num15 > 0f)
			{
				num4++;
