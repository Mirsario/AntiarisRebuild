...
namespace Antiaris.UIs
{
	public class NixieTubeSlot : UIElement
	{
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			Color newColor = new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor);
			...
			if (ItemSlot.ShiftInUse && Main.mouseItem.type <= ItemID.None)
			{
				for (int x = 0; x < 10; x++)
				{
					for (int y2 = 0; y2 < 5; y2++)
					{
						...
						int x2 = (int)(20.0 + x * 56 * (double)Main.inventoryScale);
						int y = (int)(20.0 + y2 * 56 * (double)Main.inventoryScale);
						int slot = x + y2 * 10;
						if (Main.mouseX >= x2 && Main.mouseX <= x2 + (double)Main.inventoryBackTexture.Width * Main.inventoryScale && (Main.mouseY >= y && Main.mouseY <= y + (double)Main.inventoryBackTexture.Height * Main.inventoryScale) && !PlayerInput.IgnoreMouseInterface)
						{
							if (Main.mouseLeftRelease && Main.mouseLeft)
							{
								...
								{
									SoundEngine.PlaySound(7, -1, -1, 1, 1.0f, 0.0f);
									...
									if (type > 0)
										NixieTubeUI.entity.Chip = item;
									else
										NixieTubeUI.entity.Lightbulb = item;
									if (Main.netMode == 1)
							...
							Main.mouseLeft)
					{
						if (ItemSlot.ShiftInUse)
						{
							if (!IsEmpty(item) && IsEmpty(Main.mouseItem))
							{
								DropItem(item);
								if (type > 0)
									NixieTubeUI.entity.Chip = new Item();
								else
									NixieTubeUI.entity.Lightbulb = new Item();
								if (Main.netMode == 1)
									NixieTubeUI.entity.SendClientMessage();
							}
						}
			...
			if (type > 0)
				texture2 = mod.GetTexture("UIs/Chip");
			...
			if ((type > 0 ? NixieTubeUI.entity.Chip : NixieTubeUI.entity.Lightbulb) == null || (type > 0 ? NixieTubeUI.entity.Chip : NixieTubeUI.entity.Lightbulb).type <= 0 || (type > 0 ? NixieTubeUI.entity.Chip : NixieTubeUI.entity.Lightbulb).stack <= 0)
				...
				DrawItem(spriteBatch, ref item, newPos, color, ref swapFloat);
		}
		...
		protected void DrawItem(SpriteBatch spriteBatch, ref Item item, Vector2 drawPos, Color lightColor, ref float inventoryScale, float alpha = 1.0f)
		{
			...
			if (r.Width > 32 || r.Height > 32)
				newScale = r.Width <= r.Height ? 32.0f / r.Height : 32.0f / r.Width;
			float currentScale = newScale * inventoryScale;

			Vector2 position = drawPos - r.Size() * currentScale / 2.0f;
			Vector2 origin = r.Size() * (float)(scale / 2.0 - 0.5);
