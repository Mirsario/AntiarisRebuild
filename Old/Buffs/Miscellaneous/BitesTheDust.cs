...
namespace Antiaris.Buffs.Miscellaneous
{
	public class BitesTheDust : ModBuff
	{
		public override void SetDefaults()
		{
			...
			Description.AddTranslation((int)GameCulture.CultureName.Chinese, "你的眼里有些东西...");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "???");
			Description.AddTranslation((int)GameCulture.CultureName.Russian, "В вашем глазу что-то есть...");
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			if (player.buffTime[buffIndex] == 60 && AntiarisWorld.heavenClock == 0)
			{
				SoundEngine.PlaySound(mod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Items/Detonator"), player.position);
			}
			if (player.buffTime[buffIndex] == 0 && AntiarisWorld.heavenClock == 0)
			{
				...
				SoundEngine.PlaySound(2, (int)player.position.X, (int)player.position.Y, 62);
				//vanilla code
				for (int num628 = 0; num628 < 40; num628++)
				{
					int num629 = Dust.NewDust(new Vector2(player.position.X, player.position.Y), player.width, player.height, 31, 0f, 0f, 100, default, 2f);
				...
				{
					int num631 = Dust.NewDust(new Vector2(player.position.X, player.position.Y), player.width, player.height, 6, 0f, 0f, 100, default, 3f);
					Main.dust[num631].noGravity = true;
					Main.dust[num631].velocity *= 5f;
					num631 = Dust.NewDust(new Vector2(player.position.X, player.position.Y), player.width, player.height, 6, 0f, 0f, 100, default, 2f);
				...
				for (int num632 = 0; num632 < 3; num632++)
				{
					...
					int num633 = Gore.NewGore(new Vector2(player.position.X + player.width / 2 - 24f, player.position.Y + player.height / 2 - 24f), default, Main.rand.Next(61, 64), 1f);
					...
					num633 = Gore.NewGore(new Vector2(player.position.X + player.width / 2 - 24f, player.position.Y + player.height / 2 - 24f), default, Main.rand.Next(61, 64), 2f);
					Main.gore[num633].velocity *= scaleFactor10;
					...
					num633 = Gore.NewGore(new Vector2(player.position.X + player.width / 2 - 24f, player.position.Y + player.height / 2 - 24f), default, Main.rand.Next(61, 64), 1f);
					Main.gore[num633].velocity *= scaleFactor10;
