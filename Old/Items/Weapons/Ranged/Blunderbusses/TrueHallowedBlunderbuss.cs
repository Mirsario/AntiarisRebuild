...
namespace Antiaris.Items.Weapons.Ranged.Blunderbusses
{
	public class TrueHallowedBlunderbuss : ModItem
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "真·神圣火铳");
			Tooltip.AddTranslation((int)GameCulture.CultureName.Chinese, "1、使用火铳弹当做弹药 \n2、<right> 键观察远处。 \n3、50%的几率不消耗弹药\n4、有概率发射出觉醒之弹，增加远程武器的致命一击率和伤害");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Истинный Святой мушкетон");
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			{
				Projectile.NewProjectile(position.X, position.Y, speedX * 1.4f, speedY * 1.4f, ModContent.ProjectileType<TrueBuckshot1>(), damage + 15, knockBack + 2.0f, player.whoAmI);
				return false;
			}
			return true;
		}
		...
		{
			CreateRecipe()
				.AddIngredient(null, "HallowedBlunderbuss", 1)
