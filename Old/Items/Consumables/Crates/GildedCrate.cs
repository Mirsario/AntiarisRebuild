...
namespace Antiaris.Items.Consumables.Crates
{
	public class GildedCrate : ModItem
	{
		public override void SetDefaults()
		{
			...
			item.maxStack = 99;
			item.createTile = ModContent.TileType<GildedCrate>();
			item.useTurn = true;
			item.autoReuse = true;
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "镀金板条箱");
			Tooltip.AddTranslation((int)GameCulture.CultureName.Chinese, "<right>来打开");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Позолоченный ящик");
		...
		public override void RightClick(Player player)
		{
			...
			if (Main.rand.Next(20) == 0)
				player.QuickSpawnItem(ModContent.ItemType<GiantEmerald>());
			if (Main.rand.Next(20) == 0)
				player.QuickSpawnItem(ModContent.ItemType<GiantDiamond>());
