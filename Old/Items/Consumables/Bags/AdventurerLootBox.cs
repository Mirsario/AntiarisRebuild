...
namespace Antiaris.Items.Consumables.Bags
{
	public class AdventurerLootBox : ModItem
	{
		public override void SetDefaults()
		{
			...
			item.createTile = ModContent.TileType<AdventurerLootBox>();
		}
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "冒险家的战利品箱子");
			Tooltip.AddTranslation((int)GameCulture.CultureName.Chinese, "<right>来打开");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Коробка с добычей Путешественника");
		...
		public override void RightClick(Player player)
		{
			...
			player.QuickSpawnItem(ModContent.ItemType<IronCoin>(), Main.rand.Next(1, 3));
			...
			{
				player.QuickSpawnItem(ModContent.ItemType<CalmnessRing>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<InkMonsterMask>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<FunnyMask>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<BearMask>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<MinerMask>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<CookMask>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<PlatinumAppleMask>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<PowerMask>());
			}
			if (Main.rand.Next(20) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<BizarreCap>());
			}
			...
			if (Main.rand.Next(30) == 0)
			{
				if (Main.hardMode)
				{
					player.QuickSpawnItem(ModContent.ItemType<EndlessDartPack>());
				}
			}
			if (Main.rand.Next(30) == 0)
			{
				if (Main.hardMode)
				{
					player.QuickSpawnItem(ModContent.ItemType<EndlessRocketSilo>());
				}
			}
			if (Main.rand.Next(30) == 0)
			{
			...
			{
				player.QuickSpawnItem(ModContent.ItemType<SpearofDespair>());
			}
			if (Main.rand.Next(40) == 0 && Main.hardMode)
			{
				player.QuickSpawnItem(ModContent.ItemType<CursedHand>());
			}
			if (Main.rand.Next(10) == 0)
			{
			...
			if (Main.rand.Next(7) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<HealthElixir>(), Main.rand.Next(2, 5));
			}
			if (Main.rand.Next(7) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<ManaElixir>(), Main.rand.Next(2, 5));
			}
			if (Main.rand.Next(6) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<RuneStone>(), Main.rand.Next(3, 5));
			}
			if (Main.rand.Next(25) == 0 && Main.hardMode)
			{
				player.QuickSpawnItem(ModContent.ItemType<CannonofNightmares>());
			}
			if (Main.rand.Next(25) == 0)
			{
				player.QuickSpawnItem(ModContent.ItemType<ChestFounder>());
			}
			if (Main.rand.Next(15) == 0 && Main.hardMode)
			{
				player.QuickSpawnItem(ModContent.ItemType<SuspiciousFragment>(), Main.rand.Next(1, 3));
			}
		}
	}
}
