...
namespace Antiaris.NPCs.Town
{
	...
	{
		public override string Texture => "Antiaris/NPCs/Town/Adventurer";
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "冒险家");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Путешественник");
		...
		{
			return ModContent.GetInstance<AntiarisWorld>().savedAdventurer || (!NPC.AnyNPCs(ModContent.NPCType<Adventurer>()) && !NPC.AnyNPCs(ModContent.NPCType<FrozenAdventurer>()));
		}
		...
		public override void TownNPCAttackProj(ref int projType, ref int attackDelay)
		{
			...
			{
				case 2:
					projType = ModContent.ProjectileType<AdventurerAttack3>();
					attackDelay = 30;
					if (npc.localAI[3] == attackDelay - 1)
						SoundEngine.PlaySound(2, npc.Center, 43);
					break;
				case 1:
					projType = ModContent.ProjectileType<AdventurerAttack2>();
					attackDelay = 30;
					if (npc.localAI[3] == attackDelay - 1)
						SoundEngine.PlaySound(2, npc.Center, 43);
					break;
				default:
					projType = ModContent.ProjectileType<AdventurerAttack1>();
					attackDelay = 30;
					if (npc.localAI[3] == attackDelay - 1)
						SoundEngine.PlaySound(2, npc.Center, 43);
					break;
		...
		public override void SetupShop(Chest shop, ref int nextSlot)
		{
			...
			shop.item[nextSlot].SetDefaults(ModContent.ItemType<AdventurerCrystal>());
			shop.item[nextSlot].shopCustomPrice = new int?(5);
			shop.item[nextSlot].shopSpecialCurrency = AntiarisMod.coin;
			nextSlot++;
			if (questSystem.CompletedTotal >= 7)
			{
				shop.item[nextSlot].SetDefaults(ModContent.ItemType<AdventurerSign>());
			...
			{
				shop.item[nextSlot].SetDefaults(ModContent.ItemType<AdventurerStar>());
			...
			{
				shop.item[nextSlot].SetDefaults(ModContent.ItemType<AdventurerSigil>());
				shop.item[nextSlot].shopCustomPrice = new int?(10);
				shop.item[nextSlot].shopSpecialCurrency = AntiarisMod.coin;
				nextSlot++;
			}
			...
			{
				shop.item[nextSlot].SetDefaults(ModContent.ItemType<AdventurerEmblem>());
				shop.item[nextSlot].shopCustomPrice = new int?(5);
				shop.item[nextSlot].shopSpecialCurrency = AntiarisMod.coin;
				nextSlot++;
			}
			...
			{
				shop.item[nextSlot].SetDefaults(ModContent.ItemType<ArchaelogistManual>());
				shop.item[nextSlot].shopCustomPrice = new int?(15);
				shop.item[nextSlot].shopSpecialCurrency = AntiarisMod.coin;
				nextSlot++;
			}
			if (Main.hardMode)
			{
				shop.item[nextSlot].SetDefaults(ModContent.ItemType<CelestialManual>());
				shop.item[nextSlot].shopCustomPrice = new int?(5);
				shop.item[nextSlot].shopSpecialCurrency = AntiarisMod.coin;
				nextSlot++;
				shop.item[nextSlot].SetDefaults(ModContent.ItemType<AdventurersMachete>());
			...
			foreach (Player player in Main.player)
			{
				if (player.active && player.talkNPC == npc.whoAmI)
				{
					...
					if (questSystem.CompletedToday)
					{
						...
						{
							case 0:
								Main.npcChatText = NoQuest1;
								return;
							case 1:
								Main.npcChatText = NoQuest2;
								return;
							default:
								Main.npcChatText = NoQuest3;
								return;
						}
					}
		...
		{
			ModContent.GetInstance<AntiarisWorld>().savedAdventurer = true;
		}
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
			{
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<AdventurerHat>(), 1, false, 0, false, false);
				}
			}
		}
	}
