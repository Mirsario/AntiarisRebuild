	...
	public class QuestSystem : ModPlayer
	{
		...
		public override void Initialize()
		{
			try
			{
				...
				Quests.Clear();
				Quest quest = new ItemQuest(Name0 + "\n\n" + Quest0, ModContent.ItemType<OldCompass>());
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name1 + "\n\n" + Quest1, ModContent.ItemType<GlacialCrystal>());
				quest.Reward = "";
				Quests.Add(quest);
				...
				quest = new ItemQuest(Name3 + "\n\n" + Quest3, ModContent.ItemType<MonsterSkull>());
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name4 + "\n\n" + Quest4, ModContent.ItemType<HarpyEgg>());
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name5 + "\n\n" + Quest5, ModContent.ItemType<GoldenApple>(), 1, 1d, "Mods.Antiaris.ThanksApple1");
				quest.Reward = "GoldenAppleMask";
				Quests.Add(quest);

				quest = new ItemQuest(Name6 + "\n\n" + Quest6, ModContent.ItemType<Necronomicon>());
				...
				int[] bird = { ModContent.NPCType<BirdTraveller>() };
				quest = new KillQuest(Name9 + "\n\n" + Quest9, bird, 5, 1d);
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name10 + "\n\n" + Quest10, ModContent.ItemType<SilkScarf>(), 1, 1d);
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name11 + "\n\n" + Quest11, ModContent.ItemType<AdventurersFishingRod>(), 1, 1d);
				quest.Reward = "";
				Quests.Add(quest);

				int armsdealer = NPC.FindFirstNPC(NPCID.ArmsDealer);
				quest = new ItemQuest(Name12 + "\n\n" + Quest12, ModContent.ItemType<Bonebardier>(), 1, 1d, "Mods.Antiaris.ThanksBonebardier");
				quest.IsAvailable = () => armsdealer >= 0;
				quest.Reward = "Bonebardier";
				Quests.Add(quest);

				quest = new ItemQuest(Name13 + "\n\n" + Quest13, ModContent.ItemType<DemonWingPiece>(), 12, 1d);
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name14 + "\n\n" + Quest14, ModContent.ItemType<AdventurerChest>(), 1, 1d, "Mods.Antiaris.ThanksChest");
				quest.Reward = "SwordsmanGuide";
				Quests.Add(quest);

				quest = new ItemQuest(Name15 + "\n\n" + Quest15, ModContent.ItemType<Coconut>(), 16, 1d);
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name16 + "\n\n" + Quest16, ModContent.ItemType<SpiderMass>(), 12, 1d);
				quest.Reward = "";
				Quests.Add(quest);

				quest = new ItemQuest(Name17 + "\n\n" + Quest17, ModContent.ItemType<StolenPresent>(), 20, 1d);
				quest.IsAvailable = () => Main.xMas && NPC.downedPlantBoss;
				quest.Reward = "GelidRing";
				Quests.Add(quest);

				quest = new ItemQuest(Name18 + "\n\n" + Quest18, ModContent.ItemType<EmeraldShard>(), 12, 1d, "Mods.Antiaris.ThanksShards");
				quest.Reward = "LivingEmerald";
				Quests.Add(quest);

				quest = new ItemQuest(Name19 + "\n\n" + Quest19, ModContent.ItemType<HeavenlyClock>(), 1, 0d, "Mods.Antiaris.ThanksClock");
				quest.Reward = "HeavenlyClock";
				Quests.Add(quest);

				if (AntiarisMod.TerrariaOverhaul != null)
				{
					quest = new ItemQuest(Name20 + "\n\n" + Quest20, AntiarisMod.TerrariaOverhaul.ModContent.ItemType <$1 > (), 25, 1d);
					quest.IsAvailable = () => (AntiarisMod.TerrariaOverhaul != null);
					quest.Reward = "";
					Quests.Add(quest);
				}
			}
		...
		public bool CheckQuest()
		{
			...
			return quest.CheckCompletion(Main.LocalPlayer);
		}
		...
		public void SpawnReward(NPC npc)
		{
			try
			{
				if (!BrokenRod)
				{
					SoundEngine.PlaySound(24, -1, -1, 1);
					int number = 0;
					number = Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height,
						ModContent.ItemType<AdventurerLootBox>(), 1, false, 0, false, false);
					...
					if (Main.netMode == NetmodeID.MultiplayerClient && number2 >= 0)
						NetMessage.SendData(MessageID.SyncItem, -1, -1, null, number2, 1f, 0.0f, 0.0f, 0, 0, 0);
				}
				BrokenRod = false;
			}
			catch (Exception exception)
			...
			if (remoteClient == -1)
				for (int remoteClient1 = 0; remoteClient1 < 255; ++remoteClient1)
				{
					if (Netplay.Clients[remoteClient1].State == 10)
						NetMessage.SendData(MessageID.AnglerQuest, remoteClient1, -1, NetworkText.FromLiteral(Main.player[remoteClient1].name), Main.LocalPlayer.GetModPlayer<QuestSystem>(mod).CurrentQuest, 0.0f, 0.0f, 0.0f, 0, 0, 0);
				}
			else
			{
				if (Netplay.Clients[remoteClient].State != 10)
					return;
				NetMessage.SendData(MessageID.AnglerQuest, remoteClient, -1, NetworkText.FromLiteral(Main.player[remoteClient].name), Main.LocalPlayer.GetModPlayer<QuestSystem>(mod).CurrentQuest, 0.0f, 0.0f, 0.0f, 0, 0, 0);
			}
		}
		...
		public override void PostUpdate()
		{
			...
			try
			{
				if (Main.dayTime && CompletedToday)
				{
					...
					if (CurrentQuest == 16 && AntiarisMod.TerrariaOverhaul == null)
					{
						CurrentQuest = -1;
					}
				}
		...
		public override void OnHitNPC(Item item, NPC target, int damage, float knockback, bool crit)
		{
			try
			{
				...
				{
					foreach (int i in (GetCurrentQuest() as KillQuest).TargetType)
