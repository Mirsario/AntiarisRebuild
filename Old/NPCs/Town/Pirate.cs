...
namespace Antiaris.NPCs.Town
{
	public class Pirate : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "船长");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Пират");
		}
		...
		public class PirateQuestSystem : ModPlayer
		{
			...
			{
				Quests.Clear();
				Quests.Add(new Quest("Mods.Antiaris.PirateQuest", ModContent.ItemType<MagicalAmulet>(), 1, 1d, "Mods.Antiaris.PirateThanks"));
			}
			...
			public int currentPirateQuest
			{
				get
				{
					return CurrentPirateQuest;
				}
				set
				{
					CurrentPirateQuest = value;
				}
			}
			...
			public bool CheckQuest()
			{
				if (CurrentPirateQuest == -1)
					return false;
				Quest quest = Quests[CurrentPirateQuest];
			...
			public void CompleteQuest()
			{
				...
				ModContent.GetInstance<AntiarisWorld>().FinishedPirateQuest = true;
			}
			...
			public void SpawnReward(NPC npc)
			{
				switch (CurrentPirateQuest)
				{
					case 0:
						int number = 0;
						number = Item.NewItem(npc.position, npc.Size, ModContent.ItemType<BrokenDavyKey>(), 1, false, 0, false, false);
