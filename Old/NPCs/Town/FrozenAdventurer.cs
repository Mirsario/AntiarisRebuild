...
namespace Antiaris.NPCs.Town
{
	public class FrozenAdventurer : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "被冻住的冒险家");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Замерзший путешественник");
		...
		public override void AI()
		{
			...
			foreach (Player player in Main.player)
			{
				if (!player.active)
					continue;
				if (player.inventory[player.selectedItem].type == ModContent.ItemType<SteelShovel>())
				{
					if (player.itemAnimation > 0 && timer <= 0.0f)
					{
						...
						if (FirstRectangle.Intersects(SecondRectangle) && (npc.noTileCollide || Collision.CanHit(player.position, player.width, player.height, npc.position, npc.width, npc.height)))
						{
							...
							{
								timer = 60.0f;
								SoundEngine.PlaySound(2, (int)player.position.X, (int)player.position.Y, 48);
								for (int i = 0; i < 20; i++)
		...
		public void WakeUp()
		{
			...
			if (Main.netMode == NetmodeID.SinglePlayer)
				npc.Transform(ModContent.NPCType<Adventurer>());
			else
			{
