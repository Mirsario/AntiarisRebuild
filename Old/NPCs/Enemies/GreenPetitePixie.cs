...
{
	public class GreenPetitePixie : ModNPC
	{
		private int frame = 0;
		...
		{
			DisplayName.SetDefault("Green Petite Pixie");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Зеленая маленькая пикси");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "绿色迷你精灵");
		...
		public override void AI()
		{
			...
			{
				int dust = Dust.NewDust(npc.position, npc.width, npc.height, 61, 0.0f, 0.0f, 200, default, 1.0f);
			...
			if (Main.npc[ai1].active)
			{
				...
				if (Main.npc[ai1].oldPos[1] != Vector2.Zero && Main.npc[ai1].type == ModContent.NPCType<GreenPixie>())
				{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			{
				for (int k = 0; k < 10; k++)
					Dust.NewDust(npc.position, npc.width, npc.height, 61, 2.5f * hitDirection, -2.5f, 0, default, 0.7f);
			}
			else
			{
				for (int k = 0; k < 50; k++)
