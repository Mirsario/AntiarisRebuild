...
namespace Antiaris.NPCs.Enemies
{
	public class SporeCarrier : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			banner = npc.type;
			bannerItem = ModContent.ItemType<SporeCarrierBanner>();
		}

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Spore Carrier");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "孢子带菌者");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Носитель спор");
			Main.npcFrameCount[npc.type] = 3;
		}

		/*public void OverhaulInit()
		{
			this.SetTag(NPCTags.Zombie);
		}*/
		...
		public override void AI()
		{
			...
			if (timer == 120 && Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				for (int i = 0; i < ProjectileAmount; i++)
				{
					...
					int a = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, value17.X, value17.Y, ProjectileID.SporeCloud, 1, 1f);
		...
		public override void NPCLoot()
		{
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<RavenousSpores>(), 1, false, 0, false, false);
				}
		...
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			...
			return (AntiarisMod.NormalSpawn(spawnInfo) && NPC.downedBoss2 && AntiarisMod.NoZoneAllowWater(spawnInfo)) && spawnInfo.player.ZoneJungle && y < Main.worldSurface ? 0.02f : 0f;
		}
	}
}