...
{
	public class PetrousKnight1 : ModNPC
	{
		public override void SetDefaults()
		{
			...
			npc.rarity = 1;
			banner = npc.type;
			bannerItem = ModContent.ItemType<PetrousKnight1Banner>();
		}
		...
		public override void NPCLoot()
		{
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<StoneHammer1>(), 1, false, 0, false, false);
				}
			}
		}
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
			{
				for (int k = 0; k < 50; k++)
				{
					Dust.NewDust(npc.position, npc.width, npc.height, 62, 2.5f * hitDirection, -2.5f, 0, default, 1.2f);
				}
