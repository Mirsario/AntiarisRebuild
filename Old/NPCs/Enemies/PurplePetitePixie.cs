...
{
	public class PurplePetitePixie : ModNPC
	{
		private int frame = 0;
		...
		{
			DisplayName.SetDefault("Purple Petite Pixie");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Фиолетовая маленькая пикси");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "紫色迷你精灵");
		...
		public override void AI()
		{
			...
			if (Main.npc[ai1].active)
			{
				...
				if (Main.npc[ai1].oldPos[1] != Vector2.Zero && Main.npc[ai1].type == ModContent.NPCType<PurplePixie>())
				{
