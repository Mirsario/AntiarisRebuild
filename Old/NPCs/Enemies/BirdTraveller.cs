...
namespace Antiaris.NPCs.Enemies
{
	public class BirdTraveller : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "荼毒女王鸟");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Птица-путешественник");
		...
		public override void AI()
		{
			...
			if (timer >= 180f && timer % 20f == 0f)
			{
				...
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, vector2_2.X, vector2_2.Y, ModContent.ProjectileType<TravellerFeather>(), npc.damage / 3 + 15, 5.0f, 0, 0.0f, 0.0f);
			}
			if (timer >= 260.0f)
				timer = 0.0f;
		}
		...
		public override void NPCLoot()
		{
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<TravellersFeather>(), 1, false, 0, false, false);
				}
			}
		}
		...
		{
			QuestSystem questSystem = spawnInfo.player.GetModPlayer<QuestSystem>(mod);
