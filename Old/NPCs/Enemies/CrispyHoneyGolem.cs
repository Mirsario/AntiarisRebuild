...
namespace Antiaris.NPCs.Enemies
{
	public class CrispyHoneyGolem : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Хрустящий медовый голем");
		...
		public override void OnHitByItem(Player player, Item item, int damage, float knockback, bool crit)
		{
			if (Main.rand.Next(4) == 0)
			{
				...
				for (int num221 = 0; num221 < num220; num221++)
				{
					...
					value17.Normalize();
					value17 *= 100 * 0.06f;
					int honey = Projectile.NewProjectile(npc.position.X, npc.position.Y, value17.X, value17.Y, ModContent.ProjectileType<HoneySpray>(), 25, 1f, 0, 0f);
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
			{
				for (int k = 0; k < 20; k++)
				{
					int dust = Dust.NewDust(npc.position, npc.width, npc.height, 138, 0f, 0f, 50, default, 1.5f);
