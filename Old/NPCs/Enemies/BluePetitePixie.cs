...
namespace Antiaris.NPCs.Enemies
{
	public class BluePetitePixie : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Голубая маленькая пикси");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "蓝色迷你精灵");
		...
		public override void AI()
		{
			...
			{
				int dust = Dust.NewDust(npc.position, npc.width, npc.height, 59, 0.0f, 0.0f, 200, default, 1.0f);
			...
			if (Main.npc[ai1].active)
			{
				...
				if (Main.npc[ai1].oldPos[1] != Vector2.Zero && Main.npc[ai1].type == ModContent.NPCType<BluePixie>())
				{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			{
				for (int k = 0; k < 10; k++)
					Dust.NewDust(npc.position, npc.width, npc.height, 59, 2.5f * hitDirection, -2.5f, 0, default, 0.7f);
			}
			else
			{
				for (int k = 0; k < 50; k++)
