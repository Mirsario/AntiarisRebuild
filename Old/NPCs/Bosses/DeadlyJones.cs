...
namespace Antiaris.NPCs.Bosses
{
	...
	public class DeadlyJones : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			bossBag = ModContent.ItemType<DeadlyJonesTreasureBag>();
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Дэдли Джонс");
		...
		/*public override void AI()
		{
			...
			var anyPlayer = Main.LocalPlayer;
			//anyPlayer.
			...
			if (aiTimer > 160 && aiTimer < 1200)
			{
				...
				if (attackID == 0)
				{
					...
					if (tornadoTimer % (Main.expertMode ? 40 : 60) == 0)
					{
						...
						NPC.NewNPC((int)((Main.player[npc.target].position.X - 500) + Main.rand.Next(1000)), (int)((Main.player[npc.target].position.Y - 500) + Main.rand.Next(1000)), ModContent.NPCType<SeaTornado>());
					}
				}
				...
				if (attackID == 2)
				{
					...
					if (teleportingTimer % 160 == 0)
					{
						...
						for (int i = 0; i < 10; i++)
						{
							int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 56, npc.velocity.X + Main.rand.Next(-10, 10), npc.velocity.Y + Main.rand.Next(-10, 10), 5, npc.color, 2.6f);
							Main.dust[dust].noGravity = true;
						}
						SoundEngine.PlaySound(SoundID.Item8, player.position);
						for (int i = 0; i < 10; i++)
						{
							int dust = Dust.NewDust(new Vector2(player.position.X, player.position.Y), player.width, player.height, 56, player.velocity.X + Main.rand.Next(-10, 10), player.velocity.Y + Main.rand.Next(-10, 10), 5, Color.LightBlue, 2.6f);
							Main.dust[dust].noGravity = true;
						}
						player.position.X = bossOldPosX;
						player.position.Y = bossOldPosY;
						...
						Vector2 velocity = AntiarisHelper.VelocityToPoint(npc.Center, AntiarisHelper.RandomPointInArea(new Vector2(player.Center.X - 10, player.Center.Y - 10), new Vector2(player.Center.X + 20, player.Center.Y + 20)), Main.expertMode ? 16 : 20);
						int shark = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, velocity.X, velocity.Y, ProjectileID.MiniSharkron, Main.rand.Next(20, 30), 1f);
						Main.projectile[shark].friendly = false;
						Main.projectile[shark].hostile = true;
						Main.projectile[shark].tileCollide = false;
						Main.projectile[shark].timeLeft = 300;
				...
				if (attackID == 3)
				{
					...
					if (soulTimer == 60)
					{
						...
						Vector2 velocity = AntiarisHelper.VelocityToPoint(player.Center, AntiarisHelper.RandomPointInArea(new Vector2(npc.Center.X - 10, npc.Center.Y - 10), new Vector2(npc.Center.X + 20, npc.Center.Y + 20)), Main.expertMode ? 1 : 2);
						int soul = Projectile.NewProjectile(player.Center.X, player.Center.Y, velocity.X, velocity.Y, ModContent.ProjectileType<StolenSoul>(), 0, 0f, Main.myPlayer);
					}
				}
			}
			if (aiTimer == 600)
