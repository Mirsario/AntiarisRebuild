...
namespace Antiaris.NPCs.Bosses
{
	public class ProtectiveStone : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "护佑魔石");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Защитный камень");
		}
		...
		public override bool PreAI()
		{
			...
			if (boss < 0 || boss >= 200 || !Main.npc[boss].active || Main.npc[boss].type != ModContent.NPCType<TowerKeeper>())
			{
				npc.active = false;
				return false;
			}
			...
			if (npc.ai[3] % 10.0 == 1.0 && Main.rand.Next(2) == 0 && Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				Projectile.NewProjectile(npc.Center.X, npc.Center.Y, start.X * speed, start.Y * speed, ModContent.ProjectileType<Energy>(), 15, 0.0f, Main.myPlayer, Main.npc[boss].Center.X, Main.npc[boss].Center.Y - 42f);
			}
			return false;
		}
