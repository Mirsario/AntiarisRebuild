...
namespace Antiaris.NPCs.Bosses
{
	...
	public class AntlionQueen : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			bossBag = ModContent.ItemType<AntlionQueenTreasureBag>();
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "蚁后");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Королева муравьиных львов");
		...
		public override void ScaleExpertStats(int playerXPlayers, float bossLifeScale)
		{
			if (playerXPlayers > 1)
			{
				npc.lifeMax = (int)(8150 + npc.lifeMax * 0.2 * playerXPlayers);
			}
			else
			{
				npc.lifeMax = 8150;
		...
		{
			ModContent.GetInstance<AntiarisWorld>().DownedAntlionQueen = true;
			if (Main.expertMode)
			{
				npc.DropBossBags();
			}
			if (!Main.expertMode)
			{
				...
				{
					case 0:
						Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<DesertRage>(), 1, false, 0, false, false);
						break;
					case 1:
						Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<ThousandNeedles>(), 1, false, 0, false, false);
						break;
					case 2:
						Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<AntlionLongbow>(), 1, false, 0, false, false);
						break;
					case 3:
						Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<AntlionStave>(), 1, false, 0, false, false);
						break;
				}
				if (Main.rand.Next(7) == 0)
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<AntlionQueenMask>(), 1, false, 0, false, false);
				}
				if (Main.rand.Next(20) == 0)
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<AntlionQueenEgg>(), 1, false, 0, false, false);
				}
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<AntlionCarapace>(), Main.rand.Next(30, 50), false, 0, false, false);
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<SandstormScroll>(), Main.rand.Next(2, 4), false, 0, false, false);
			}
			if (Main.rand.Next(10) == 0)
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<AntlionQueenTrophy>(), 1, false, 0, false, false);
			}
		}
		...
		public override void AI()
		{
			...
			this.moveSpeed = (int)((float)(npc.lifeMax / (double)npc.life) * 0.05f);
			if (player.position.X >= npc.position.X - 8.0)
				npc.spriteDirection = 1;
			else if (player.position.X < npc.position.X - 8.0)
				npc.spriteDirection = -1;
			...
			if (npc.ai[0] < 150.0 && !this.stunned && !this.sAI)
			{
				...
				if (npc.ai[1] % (Main.expertMode ? 25 : 30) == 0)
				{
					...
					Projectile.NewProjectile(x * 16 + 8, y * 16 - 56, 0f, 0f, ModContent.ProjectileType<Sandnado>(), npc.damage / 2, 0f, Main.myPlayer, 16f, 15f);
				}
				npc.netUpdate = true;
			}
			else if (npc.ai[0] >= 145.0 && !this.stunned && !this.sAI)
			{
				npc.noTileCollide = false;
				npc.velocity.X = 0f;
				npc.velocity.Y = 5f;
				...
				++npc.ai[2];
				if (npc.ai[2] > 60.0 && this.swarmerSpawnTimer > 60)
				{
					SoundEngine.PlaySound(15, (int)npc.position.X, (int)npc.position.Y, 0);
					float x = npc.Center.X + Main.rand.Next(-50, 50);
					float y = npc.Center.Y + Main.rand.Next(-50, 50);
					if (Main.tile[(int)(x / 16.0f), (int)(y / 16.0f)].active())
					{
						x = npc.Center.X + Main.rand.Next(-50, 50);
						y = npc.Center.Y + Main.rand.Next(-50, 50);
					}
					else
					{
						for (int k = 0; k < 50; k++)
						{
							int dust = Dust.NewDust(new Vector2(x, y), 16, 16, 32, 0.0f, -2.0f, 0, new Color(), 5.0f);
						...
						if (Main.rand.Next(2) == 0)
							NPC.NewNPC((int)x, (int)y, 508, 0, npc.whoAmI, 0.0f, 0.0f, 0.0f, 255);
						float newX = x + Main.rand.Next(-50, 50);
						float newY = y + Main.rand.Next(-50, 50);
						for (int k = 0; k < 50; k++)
						{
							int dust = Dust.NewDust(new Vector2(newX, newY), 16, 16, 32, 0.0f, -2.0f, 0, new Color(), 5.0f);
							Main.dust[dust].noGravity = true;
							Dust dust1 = Main.dust[dust];
							dust1.position.X = dust1.position.X + (Main.rand.Next(-50, 51) / 20 - 1.5f);
							Dust dust2 = Main.dust[dust];
						...
						if (Main.rand.Next(2) == 0)
							NPC.NewNPC((int)newX, (int)newY, 509, 0, npc.whoAmI, 0.0f, 0.0f, 0.0f, 255);
					}
			...
			if (npc.ai[3] >= 10.0 && npc.ai[3] < 20.0)
			{
				this.mv += 1 + (npc.life > npc.lifeMax / 2 ? 0 : 1) + (npc.life > npc.lifeMax / 3 ? 0 : 1);
				this.frame = 0;
				npc.noTileCollide = true;
				if (this.mv >= 150 && this.mv < 500)
				{
					...
					if (this.mv > 350)
					{
						...
						npc.velocity.X = 0.0f;
						npc.velocity.Y = 20f;
						if (npc.position.Y < player.position.Y + 500.0)
						{
							this.mv = 425;
						}
					}
				...
				else if (this.mv >= 650 && this.mv < 1000)
				{
					...
					if (this.mv > 850)
					{
						...
						npc.velocity.X = 20f;
						npc.velocity.Y = 0.0f;
						if (npc.position.X < player.position.X + 500.0)
						{
							this.mv = 925;
						}
					}
			...
			else if (npc.ai[3] >= 20.0 && npc.ai[3] < 30.0)
			{
				AntiarisHelper.MoveTowards(npc, target, Vector2.Distance(target, npc.Center) > 500 ? (Main.expertMode ? 32f : 30f) : (Main.expertMode ? 13f : 11f), 30f);
				this.mv += (int)0.2 + (npc.life > npc.lifeMax / 2 ? 0 : 1) + (npc.life > npc.lifeMax / 3 ? 0 : 1);
				this.frame = 0;
				if (this.mv >= 100 && this.mv < 175)
				{
					if (this.mv % 10 == 0)
					{
						...
						shootVel.Normalize();
						shootVel *= 28f;
						int k = Projectile.NewProjectile(shootPos, shootVel, ProjectileID.SandBallFalling, npc.damage / 2, 5f, Main.myPlayer);
			...
			else if (npc.ai[3] >= 30.0)
			{
				this.mv += 2 + (npc.life > npc.lifeMax / 2 ? 0 : 1) + (npc.life > npc.lifeMax / 3 ? 0 : 1);
				this.frame = 0;
				if (this.mv % 500 == 0)
				{
					SoundEngine.PlaySound(15, (int)npc.position.X, (int)npc.position.Y, 0);
					NPC.NewNPC((int)(player.Center.X - 1200f), (int)(player.Center.Y - 1000f), 508, 0, npc.whoAmI, 0.0f, 0.0f, 0.0f, byte.MaxValue);
					NPC.NewNPC((int)(player.Center.X + 1200f), (int)(player.Center.Y - 1000f), 508, 0, npc.whoAmI, 0.0f, 0.0f, 0.0f, byte.MaxValue);
				}
			...
			if (npc.life <= npc.lifeMax * 0.15f)
			{
				if (Main.expertMode)
				{
					...
					this.end = true;
					++this.aiSECOND;
					if (this.aiSECOND % 2 == 0)
					{
						Vector2 ShootPos = player.position + new Vector2(Main.rand.Next(-1000, 1000), -1000);
						var ShootVel = new Vector2(Main.rand.NextFloat(-3f, 3f), Main.rand.NextFloat(15f, 20f));
						int k = Projectile.NewProjectile(ShootPos, ShootVel, ProjectileID.SandBallFalling, npc.damage / 4, 1f);
						Main.projectile[k].friendly = false;
						Main.projectile[k].hostile = true;
						Main.projectile[k].scale = 1.4f;
						Main.projectile[k].tileCollide = false;
					}
					if (this.aiSECOND % 120 == 0)
					{
						this.aiSECOND = 0;
						int y = (int)(npc.Center.Y / 16f);
						int x = (int)(npc.Center.X / 16f);
					...
					npc.netUpdate = true;
					if (player.position.Y < npc.position.Y + 450.0f)
						npc.velocity.Y = npc.velocity.Y - ((npc.velocity.Y > 0.0f) ? 0.8f : 0.07f);
					if (player.position.Y > npc.position.Y + 450.0f)
						npc.velocity.Y = npc.velocity.Y - ((npc.velocity.Y < 0.0f) ? 0.8f : 0.07f);
					if (npc.spriteDirection == -1 && npc.velocity.X > -12.0)
					{
						npc.velocity.X -= 0.1f;
					...
					else if (npc.spriteDirection == 1 && npc.velocity.X < 12.0)
					{
						npc.velocity.X += 0.1f;
						if (npc.velocity.X < -12.0)
							npc.velocity.X += 0.1f;
		...
		private void StartSandstorm()
		{
			...
			Sandstorm.TimeLeft = (int)(3600.0 * (8.0 + Main.rand.NextFloat() * 16.0));
