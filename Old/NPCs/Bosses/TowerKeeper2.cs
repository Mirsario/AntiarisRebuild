...
namespace Antiaris.NPCs.Bosses
{
	...
	public class TowerKeeper2 : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			bossBag = ModContent.ItemType<TowerKeeperTreasureBag2>();
		...
		public override void ScaleExpertStats(int playerXPlayers, float bossLifeScale)
		{
			if (playerXPlayers > 1)
			{
				npc.lifeMax = (int)(12000 + npc.lifeMax * 0.2 * playerXPlayers);
			}
			else
			{
				npc.lifeMax = 12000;
		...
		public override void SetStaticDefaults()
		{
			...
			DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "守塔魔像");
			DisplayName.AddTranslation((int)GameCulture.CultureName.Russian, "Хранитель башни");
		}
		...
		{
			ModContent.GetInstance<AntiarisWorld>().DownedTowerKeeper = true;
			if (Main.expertMode)
			{
				npc.DropBossBags();
			}
			if (!Main.expertMode)
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<ShadowChargedCrystal>(), Main.rand.Next(15, 24), false, 0, false, false);
				...
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<TowerKeeperMask2>(), 1, false, 0, false, false);
				}
				if (Main.rand.Next(7) == 0)
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<GuardianHeart2>(), 1, false, 0, false, false);
				}
			}
			if (Main.rand.Next(10) == 0)
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<TowerKeeperTrophy2>(), 1, false, 0, false, false);
			}
		}
		...
		public override void AI()
		{
			...
			if (!this.secondState)
			{
				...
				for (int count = 0; count < 10; count++)
				{
					...
					NPC.NewNPC((int)spawn.X, (int)spawn.Y, ModContent.NPCType<ProtectiveStone2>(), 0, npc.whoAmI, 0.0f, count, 0.0f, 255);
				}
			...
			if (npc.ai[0] < 350.0 && !this.stunned)
			{
				...
				for (int k = 0; k < 5 * (npc.ai[0] / 50); k++)
				{
					...
					if (npc.ai[0] % 2 == 1)
						scale = 0.65f;
					int index2 = Dust.NewDust(npc.Center + new Vector2(0.0f, -20f), 24, 24, 62, npc.velocity.X / 2f, npc.velocity.Y / 2f, 0, new Color(), 1f);
				...
				if (npc.ai[1] % 200.0 == 0 && npc.ai[1] <= 399.0)
				{
					...
					if (this.attackTimer <= 2)
					{
						...
						for (int k = 0; k < (Main.expertMode ? 5 : 3); k++)
							Projectile.NewProjectile(shootPos.X + -100 * npc.direction + Main.rand.Next(-40, 41), shootPos.Y - Main.rand.Next(-50, 40), shootVel.X, shootVel.Y, ModContent.ProjectileType<GolemCrystal2>(), npc.damage / 3, 5f);
					}
					else
					{
						if (Main.expertMode)
						{
							for (int i = 0; i < 7; i++)
								Projectile.NewProjectile((int)((player.position.X - 50) + Main.rand.Next(100)), (int)((player.position.Y - 50) + Main.rand.Next(100)), 0.0f, 0.0f, ModContent.ProjectileType<TowerKeeper2Sheet>(), npc.damage / 3, 4.5f);
						}
						for (int k = 0; k < (Main.expertMode ? 8 : 5); k++)
						{
							...
							Projectile.NewProjectile(shootPos, shootVel, ModContent.ProjectileType<GolemCrystal2>(), npc.damage / 2, 4.5f);
						}
						SoundEngine.PlaySound(2, (int)npc.position.X, (int)npc.position.Y, 88);
						this.attackTimer = 0;
					}
