	...
	public class LightningBolt
	{
		...
		public void Extend(Vector2 dest)
		{
			if (this.Segments.Count == 0)
				this.Segments = this.CreateBolt(this.Source, dest);
			else
				this.Segments.AddRange(this.CreateBolt(this.Segments[this.Segments.Count - 1].B, dest));
		}

		public void Draw(SpriteBatch spriteBatch, Color color = default)
		{
			foreach (Line segment in this.Segments)
				segment.Draw(spriteBatch, color);
		}

		public void moveTo(Vector2 source)
		{
			Vector2 move = source - this.Source;
			foreach (Line segment in this.Segments)
			{
				segment.A += move;
				segment.B += move;
		...
		public void moveTo(Vector2 source, Vector2 destination)
		{
			this.moveTo(source);
			if (this.Segments.Count > 0)
				this.Segments[this.Segments.Count - 1].B = destination;
		}
	}
}
